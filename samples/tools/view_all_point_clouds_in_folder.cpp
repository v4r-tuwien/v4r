...
int main(int argc, char** argv) {
  ...
  if (files_intern.empty()) {
    std::cerr
        ...
        << "Usage " << argv[0] << "  -path <folder name>" << std::endl
        << "  [-rows <number of rows used for displaying clouds ( default: " << i_rows << ")>]" << std::endl
        << "  [-center <if true, centers the point cloud in its displaying window based on its centroid ( default: "
        << center << ")>]" << std::endl
        ...
        << "  [-cloud_prefix <prefix of cloud filenames for displaying ( default: " << cloud_prefix
        << "  assumes .pcd file extension)>]" << std::endl
        << "  [-pose_prefix <prefix of pose filenames for displaying ( default: " << pose_prefix
  ...
  for (size_t file_id = 0; file_id < files_intern.size(); file_id++) {
    if (files_intern[file_id].find(indices_prefix) ==
        std::string::npos) {  // this checks if the .pcd file has indices or cloud data stored
  ...
  for (size_t file_id = 0; file_id < cloud_files.size(); file_id++) {
    ...
    if (multi_view) {  // Setting up the visualization
      ...
      vis.createViewPort(float(col_id) / cols, float(row_id) / rows, float(col_id + 1.0) / cols,
                         float(row_id + 1.0) / rows, viewport[file_id]);
