...
int main(int argc, char** argv) {
  ...
  po::options_description desc(
      "Converter from RGB-D pointclouds (XYZRGB) into rgb image and depth map "
      "image\n======================================\n**Allowed options");
  desc.add_options()("help,h", "produce help message")("input_dir,i", po::value<std::string>(&input_dir)->required(),
                                                       "input directory containing the .pcd files to be converted")(
  ...
  for (size_t sub_folder_id = 0; sub_folder_id < sub_folder_names.size(); sub_folder_id++) {
    ...
    for (size_t v_id = 0; v_id < views.size(); v_id++) {
      ...
      // save to disk
      std::stringstream fn;
      fn << output_dir << "/" << id << "_rgb.png";
      cv::imwrite(fn.str(), rgb);
      fn.str("");
      fn << output_dir << "/" << id << "_rgb_cropped.png";
      cv::imwrite(fn.str(), rgb_cropped);
      fn.str("");
      fn << output_dir << "/" << id << "_depth.png";
      cv::imwrite(fn.str(), depth);
      fn.str("");
      fn << output_dir << "/" << id << "_depth_cropped.png";
