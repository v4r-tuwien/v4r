...
void removeNaNPoints(pcl::PointCloud<PointT> &cloud) {
  size_t kept = 0;
...
template <typename PointT>
void save(const pcl::PointCloud<PointT> &cloud, const std::string &out_dir) {
  ...
  do {
    std::stringstream fn;
    fn << out_path_bf.string() << std::setfill('0') << std::setw(5) << counter++ << ".pcd";
    out_cloud_fn = fn.str();
...
int main(int argc, const char *argv[]) {
  ...
  desc.add_options()("help,h", "produce help message")("input,i", po::value<std::string>(&input_dir)->required(),
                                                       "input file path (.ply) or folder containing files")(
      "out_dir,o", po::value<std::string>(&out_dir)->default_value("/tmp/model_database/"),
      "output directory to store the rendered point clouds in the recognition model structure")(
      ...
      "radius_sphere,r",
      po::value<float>(&radius_sphere)->default_value(radius_sphere, boost::str(boost::format("%.2e") % radius_sphere)),
      "defines the radius of the sphere used for rendering")("width", po::value<size_t>(&width)->default_value(width),
      ...
      "generate_organized", po::value<bool>(&gen_organized)->default_value(gen_organized),
      "if false, removes NaN points from the rendered point cloud");
  po::variables_map vm;
  ...
  for (const std::string class_name : class_names) {
    ...
    for (const std::string &instance_name : instance_names) {
      ...
      LOG(INFO) << "Rendering file " << filename << " ( with color? " << model.hasColor()
                << ").";  // test if the model has colored elements(note! no textures supported yet.... only colored
                          // polygons)z
