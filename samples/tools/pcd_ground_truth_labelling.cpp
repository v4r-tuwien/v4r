...
class PcdGtAnnotator {
 ...
 private:
  ...
  std::vector<std::vector<std::vector<bool>>> is_pixel_annotated_for_model_in_view_;  /// @brief outer loop: model id,
  /// middle loop: view id, inner
  /// loop: pixel id
 ...
 public:
  ...
  float threshold_;
  float f_;
  bool first_view_only_;  // has been used for RA-L 16 paper, where we were interested in the visible object in the
  // first viewpoint
  ...
  void init_source() {
    ...
    source_.reset(new v4r::Source<PointT>(source_param));
    source_->init(models_dir_);
  }

  void annotate(const bf::path &scenes_dir, const std::string &scene_id = "");
  void visualize();
  void clear();
  void save_to_disk(const bf::path &path);
...
void PcdGtAnnotator<PointT>::annotate(const bf::path &scenes_dir, const std::string &scene_id) {
  ...
  if (!scene_view.empty()) {
    ...
    const bf::path annotations_dir = gt_dir_ / scene_id;
    ...
    for (size_t s_id = 0; s_id < scene_view.size(); s_id++) {
      ...
      const bf::path scene_full_file_path = scene_full_path / scene_view[s_id];
      pcl::io::loadPCDFile(scene_full_file_path.string(), *reconstructed_scene_);
      View view;
      ...
      for (size_t gt_id = 0; gt_id < gt_file.size(); gt_id++) {
        ...
        if (gt_fn.compare(0, scene_file_wo_ext.size(), scene_file_wo_ext) == 0 &&
          ...
          const bf::path gt_full_file_path = annotations_dir / gt_fn;
          ...
          out_fn << out_dir_ << "/" << setfill('0') << setw(10) << unique_id_ << ".txt";
          ...
          label_file.open((out_dir_ / "label.anno").string(), std::ofstream::out | std::ofstream::app);
          label_file << scenes_dir << " " << scene_id << " " << scene_view[s_id] << " " << setfill('0') << setw(10)
                     << unique_id_ << " " << model_name << std::endl;
          label_file.close();
          unique_id_++;
        }
      }
...
void PcdGtAnnotator<PointT>::visualize() {
  ...
  std::vector<int> viewports =
      v4r::pcl_visualizer::visualization_framework(*vis_, 1, model_id_.size() + 1, subwindow_titles);
...
int main(int argc, char **argv) {
  ...
  desc.add_options()("help,h", "produce help message")("scenes_dir,s", po::value<bf::path>(&scene_dir)->required(),
      ...
      "first_view_only", po::bool_switch(&annotator.first_view_only_),
      "if true, outputs the visible object model in the first view only");
