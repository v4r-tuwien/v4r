...
int main(int argc, char** argv) {
  ...
  desc.add_options()("help,h", "produce help message")(
      ...
      "positives (noise increase with z)")("out_dir,o", po::value<std::string>(&out_dir)->default_value(out_dir),
  ...
  for (const std::string& sub_folder_name : sub_folder_names) {
    ...
    for (size_t v_id = 0; v_id < views.size(); v_id++) {
      ...
      for (size_t m_id = 0; m_id < ohs.size(); m_id++) {
        ...
        out_fn << out_path << "/" << views[v_id].substr(0, views[v_id].length() - 4) << "_" << model_id << "_"
