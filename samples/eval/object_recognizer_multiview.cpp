...
int main(int argc, char** argv) {
  ...
  std::map<std::string, size_t> rec_models_per_id_;  // just to have a unique filename when writing pose of objects to
                                                     // disk (in case of multiple instances detected in one scene)

  google::InitGoogleLogging(argv[0]);

  po::options_description desc(
      "Multiview Object Instance Recognizer\n======================================**Reference(s): Faeulhammer et al, "
      "ICRA / MVA 2015\n **Allowed options");
  ...
  for (size_t sub_folder_id = 0; sub_folder_id < sub_folder_names.size(); sub_folder_id++) {
    ...
    for (size_t v_id = 0; v_id < views.size(); v_id++) {
      ...
      pcl::StopWatch watch;
      mv_r.recognize();
      v4r::io::writeFloatToFile(out_path + "/" + views[v_id].substr(0, views[v_id].length() - 4) + "_time.nfo",
