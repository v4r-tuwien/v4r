...
int main(int argc, char** argv) {
  ...
  int filter_boundary_pts = 7;  //  according to the edge types defined in pcl::OrganizedEdgeBase (EDGELABEL_OCCLUDING
                                //  | EDGELABEL_OCCLUDED | EDGELABEL_NAN_BOUNDARY)
  float chop_z = 3.5f;
  ...
  std::vector<typename v4r::KeypointExtractor<PointT>::Ptr> kp_extractors =
      v4r::initKeypointExtractors<PointT>(kp_extraction_type, to_pass_further);
  typename v4r::NormalEstimator<PointT>::Ptr normal_estimator =
      v4r::initNormalEstimator<PointT>(normal_estimation_type, to_pass_further);
  //    typename v4r::KeypointExtractor<PointT>::Ptr shot = v4r::initKeypointExtractor<PointT>( segmentation_method,
  //    to_pass_further);
    ...
    for (size_t v_id = 0; v_id < views.size(); v_id++) {
...

      {
        if (use_sift) {
          ...
          for (typename v4r::KeypointExtractor<PointT>::Ptr ke : kp_extractors) {
            ...
            info_txt << ke->getKeypointExtractorName() << " keypoint extraction";
      ...
      if (filter_planar) {
        ...
        *IndicesPtr = kp_indices;
        ...
        std::cout << "Kept " << kp_is_kept.count() << " points from " << kp_indices.size() << " after planar filtering."
                  << std::endl;
      ...
      if (filter_boundary_pts) {
        ...
        cv::Mat element =
            cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(2 * boundary_width + 1, 2 * boundary_width + 1),
        ...
        std::cout << "Kept " << kp_is_kept.count() << " points from " << kp_indices.size()
                  << " after filtering boundary pts." << std::endl;
      }
      ...
      if (visualize) {
        ...
        vis->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, vis_pt_size,
                                              "keypoints_cloud", vp2);
