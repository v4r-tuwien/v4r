...
void save_bb_image(const pcl::PointCloud<PointT> &cloud, const std::vector<std::vector<int>> &segments,
                   const std::string &filename_prefix, int margin = 0) {
  for (size_t i = 0; i < segments.size(); i++) {
    std::stringstream filename;
    filename << filename_prefix << "_" << std::setfill('0') << std::setw(5) << i << ".jpg";
