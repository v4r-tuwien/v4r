 ...
 * Example command:
 *
 * --do_erosion 1 --radius 0.005 --dot_product 0.99 --normal_method 0 --chop_z 2 --transfer_latest_only 0
 * --scenes_dir media/Data/datasets/TUW/test_set
 * --input_mask_dir media/Data/datasets/TUW/incremental_object_segmentation/ground_truth_for_first_frame_only
 * --output_dir home/thomas/Desktop/out_test/ -v
 *
 */
...
int main(int argc, char** argv) {
  ...
  bf::path scene_dir;
  bf::path output_dir = "dol";
  bf::path input_mask_dir;
  ...
  desc.add_options()("scenes_dir,s", po::value<bf::path>(&scene_dir)->required(),
  ...
  desc.add_options()("input_mask_dir,m", po::value<bf::path>(&input_mask_dir)->required(),
                     "directory containing the object masks used as a seed to learn the object in the first cloud");
  desc.add_options()("output_dir,o", po::value<bf::path>(&output_dir)->default_value(output_dir),
                     "Output directory where the model and parameter values will be stored");
  desc.add_options()(
  ...
  desc.add_options()(
      "normal_method",
      po::value<v4r::NormalEstimatorType>(&m.param_.normal_method_)->default_value(m.param_.normal_method_), "");
  desc.add_options()(
  ...
  ofstream param_file((output_dir / "param.nfo").string().c_str());
  ...
  for (const std::string& sub_folder_name : sub_folder_names) {
    const bf::path output_rec_model = output_dir / sub_folder_name / "models";
    v4r::io::createDirIfNotExist(output_rec_model);

    const bf::path annotations_dir = input_mask_dir / sub_folder_name;
    ...
    for (size_t o_id = 0; o_id < mask_file_v.size(); o_id++) {
      const bf::path mask_file = annotations_dir / mask_file_v[o_id];
      ...
      const bf::path scene_path = scene_dir / sub_folder_name;
      std::vector<std::string> views = v4r::io::getFilesInDirectory(scene_path, ".*.pcd", false);

      std::cout << "Learning object from mask " << mask_file.string() << " for scene " << scene_path.string()
                << std::endl;

      for (size_t v_id = 0; v_id < views.size(); v_id++) {
        const bf::path view_file = scene_path / views[v_id];
        pcl::PointCloud<PointT> cloud;
        pcl::io::loadPCDFile(view_file.string(), cloud);
      ...
      m.save_model(output_rec_model.string(), out_fn, save_views);
      //            m.write_model_to_disk(output_rec_structure.str(), output_rec_structure.str(), sub_folder_names[
      //            sub_folder_id ]);
      if (visualize)
        m.visualize();
      m.clear();
