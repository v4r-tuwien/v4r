...
Vec3b label_color[] = {Vec3b(255, 0, 0),     Vec3b(0, 255, 0),   Vec3b(0, 0, 255),    Vec3b(255, 0, 255),
                       Vec3b(0, 255, 255),   Vec3b(255, 255, 0), Vec3b(64, 255, 128), Vec3b(128, 128, 128),
                       Vec3b(255, 255, 255), Vec3b(0, 0, 0)};

class Slic_Labelling {
 protected:
  int current_label, nSclicKMax, nSclicMMax,
      nSclicK,  // superpixels number
      nSclicM,  // compactness factor
      nSclicNumOfLabels;

  bool bLeftMousePressed, is_pcd;

  Mat_<cv::Vec3b> slicSrcMat, slicImgDRAWMat;

  Mat_<int> pxLabel, slicLabelsMat;
...
int main(int argc, char* argv[]) {
  ...
  if (v4r::io::getFilesInDirectory(img_path, files_intern, "", ".*\\.(jpg|JPG|png|PNG|jpeg|JPEG|bmp|BMP|pcd|PCD)",
                                   true) != -1) {
