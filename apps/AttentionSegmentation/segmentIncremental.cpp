...
void createAttention(std::string &saliency_filename, std::vector<cv::Mat> &saliencyMaps) {
  // saliency map
...
void printUsage(char *av) {
  printf(
      "Usage: %s cloud.pcd model.txt scaling_params.txt objNumber saliency.png save_image.png times_file.txt "
      "evaluation.txt\n"
      " Options:\n"
      "   [-h] ... show this help.\n"
...
int main(int argc, char *argv[]) {
  ...
  createAttention(saliency_name, saliencyMaps);
  ...
  for (int k = 0; k < objNumber; ++k) {
    ...
    if (writeTime) {
      ...
      fprintf(f, "%20lld %20lld %20lld %20lld %20lld %20lld %20lld", tEst.times_saliencySorting.at(0),
              tEst.times_surfaceModelling.at(0), tEst.times_relationsComputation.at(0),
              tEst.times_graphBasedSegmentation.at(0), tEst.times_maskCreation.at(0), tEst.times_neigboursUpdate.at(0),
