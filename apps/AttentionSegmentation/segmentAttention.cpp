...
void createAttention(int mode, std::string &saliency_filename, pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,
                     std::vector<cv::Mat> &saliencyMaps, std::vector<cv::Point> &attentionPoints) {
  // saliency map
  if (mode == 0) {
...
void printUsage(char *av) {
  printf(
      "Usage: %s cloud.pcd model.txt scaling_params.txt mode saliency.{png,txt} [save_image.png times_file.txt "
      "evaluation.txt]\n"
      " Options:\n"
      "   [-h] ... show this help.\n"
      ...
      av);
  std::cout << " Example: " << av
            << " cloud.pcd model.txt scaling_params.txt 0 saliency.png [save_image.png times.txt evaluation.txt] "
            << std::endl;
}
}  // namespace
...
int main(int argc, char *argv[]) {
  ...
  if (writeTime) {
    ...
    if (!boost::filesystem::exists(times_filename)) {
      ...
      fprintf(f, "%12s %20s %20s %20s %20s %20s %20s %20s %10s %30s\n", "image_name", "normals", "patches",
              "patchImage", "neighbours", "border", "preRelations", "initModelSurf", "masksNum",
              "<the rest> ... total");
    } else {
      f = fopen(times_filename.c_str(), "a");
    }
    ...
    fprintf(f, "%12s %20lld %20lld %20lld %20lld %20lld %20lld %20lld %10ld", rgbd_filename_path.filename().c_str(),
            tEst.time_normalsCalculation, tEst.time_patchesCalculation, tEst.time_patchImageCalculation,
            tEst.time_neighborsCalculation, tEst.time_borderCalculation, tEst.time_relationsPreComputation,
            tEst.time_initModelSurfaces, masks.size());

    for (unsigned int k = 0; k < masks.size(); ++k) {
      fprintf(f, "%20lld %20lld %20lld %20lld %20lld %20lld %20lld", tEst.times_saliencySorting.at(k),
              tEst.times_surfaceModelling.at(k), tEst.times_relationsComputation.at(k),
              tEst.times_graphBasedSegmentation.at(k), tEst.times_maskCreation.at(k), tEst.times_neigboursUpdate.at(k),
