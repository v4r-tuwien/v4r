...
void GLViewer::update_model_cloud(const pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr &_cloud) {
  oc_mutex.lock();
  ncloud = *_cloud;
  oc_mutex.unlock();
...
void GLViewer::drawImage() {
  ...
  if (m_show_object) {
    ...
    glBegin(GL_POINTS);
    for (unsigned i = 0; i < ncloud.points.size(); i++) {
      const pcl::PointXYZRGBNormal &pt = ncloud.points[i];
      if (!std::isnan(pt.x)) {
        glColor3ub(pt.r, pt.g, pt.b);
        glVertex3f(pt.x, pt.y, pt.z);
