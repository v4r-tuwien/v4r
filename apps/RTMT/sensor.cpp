 ...
 */
Sensor::Sensor()
: m_run(false), m_run_tracker(false), m_cam_id(0), m_draw_mask(true), m_select_roi(false), m_activate_roi(false),
  roi_seed_x(320), roi_seed_y(280), u_idle_time(35 * 1000),  // 35 ms
  ...
  octree.reset(
      new pcl::octree::OctreePointCloudVoxelCentroid<pcl::PointXYZRGB,
                                                     pcl::octree::OctreeVoxelCentroidContainerXYZRGB<pcl::PointXYZRGB>>(
  ...
  while (m_run) {
    ...
    if (cloud.get() != 0) {
      ...
      if (m_select_roi)
        detectROI(kp_cloud);
      if (m_activate_roi)
        maskCloud(pose * bbox_base_transform, bb_min, bb_max, kp_cloud);
 ...
 * @param _mask
 */
void Sensor::filterCloud(const pcl::PointCloud<pcl::PointXYZRGB> &_cloud, const Eigen::Matrix4f &_pose,
 ...
 * @param _sf_pose
 */
void Sensor::initCloud(const pcl::PointCloud<pcl::PointXYZRGB> &_cloud, const Eigen::Matrix4f &_pose,
 ...
 * @param _sf_pose
 */
void Sensor::integrateData(const pcl::PointCloud<pcl::PointXYZRGB> &_cloud, const Eigen::Matrix4f &_pose,
                           v4r::DataMatrix2D<Surfel> &_sf_cloud, Eigen::Matrix4f &_sf_pose) {
  if (cam.empty())
  ...
  for (unsigned v = 0; v < _cloud.height; v++) {
    for (unsigned u = 0; u < _cloud.width; u++) {
      ...
      if (fabs(dw) <= std::numeric_limits<float>::epsilon()) {
        ...
        sf.g = _pt.g;
        sf.b = _pt.b;
        if (sf.weight < max_integration_frames)
          sf.weight += 1;
      }
    }
  }
 ...
 * @return type
 */
int Sensor::selectFrames(const pcl::PointCloud<pcl::PointXYZRGB> &_cloud, int _cam_id, const Eigen::Matrix4f &_pose,
  ...
  if (cam_tracker_params.log_point_clouds && _cam_id >= 0) {
    ...
    if (z >= cameras.size()) {
      ...
      emit printStatus(std::string("Status: Selected ") + v4r::toString(log_clouds->size(), 0) +
 ...
 * @param clouds
 */
void Sensor::renewPrevCloud(const std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>> &poses,
...
void Sensor::maskCloud(const Eigen::Matrix4f &_pose, const Eigen::Vector3f &_bb_min, const Eigen::Vector3f &_bb_max,
                       v4r::DataMatrix2D<Eigen::Vector3f> &_cloud) {
