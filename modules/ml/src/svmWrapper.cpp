...
void svmClassifier::train(const Eigen::MatrixXf &training_data, const Eigen::VectorXi &training_label) {
  ...
  if (!param_.svm_.probability && param_.knn_ > 1) {
    LOG(WARNING) << "KNN set with k>1 but probability estimate is turned off. Will turn on SVM probability to estimate "
                    "not only winner.";
  ...
  for (int i = 0; i < svm_prob->l; i++)
    svm_prob->x[i] = new ::svm_node[num_attributes +
                                    1];  // add one additional dimension and set that index to -1 (libsvm requirement)
  ...
  if (v4r::io::existsFile(param_.filename_)) {
    ...
    if (param_.do_cross_validation_ > 1) {
      ...
      for (double C = param_.cross_validation_range_C_[0]; C <= param_.cross_validation_range_C_[1];
           C *= param_.cross_validation_range_C_[2]) {
        for (double gamma = param_.cross_validation_range_gamma_[0]; gamma <= param_.cross_validation_range_gamma_[1];
             gamma *= param_.cross_validation_range_gamma_[2]) {
          param_.svm_.C = C;
          ...
          LOG(INFO) << "Accuracy for parameters C=" << param_.svm_.C << " and gamma=" << param_.svm_.gamma << ": "
                    << performance << " with confusion matrix: " << std::endl
                    << conf_matrix << std::endl;
      ...
      LOG(INFO) << "Best parameters achieved from cross-validation: C=" << param_.svm_.C
                << " and gamma=" << param_.svm_.gamma;
    }
