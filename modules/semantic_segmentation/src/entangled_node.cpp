...
: mParent(nullptr), mOnFrontier(true), mIsSplitNode(false), mSplitFeature(nullptr), mDepth(-1), mLeftChildIdx(-1),
  mRightChildIdx(-1) {}

// constructor for root node
EntangledForestNode::EntangledForestNode(EntangledForestData *data)
: mParent(nullptr), mOnFrontier(true), mIsSplitNode(false), mSplitFeature(nullptr), mDepth(0), mLeftChildIdx(-1),
  mRightChildIdx(-1) {
  // set begin and end iterator
  data->GetBeginAndEndIterator(mTrainingDataIterators.first, mTrainingDataIterators.second);
  data->CalculateLabelDistributions(mTrainingDataIterators.first, mTrainingDataIterators.second, mAbsLabelDistribution,
                                    mLabelDistribution);
}

// constructor for child node
EntangledForestNode::EntangledForestNode(EntangledForestData *data, ClusterIdxItr dataStart, ClusterIdxItr dataEnd,
                                         EntangledForestNode *parent)
: mParent(parent), mOnFrontier(true), mIsSplitNode(false), mSplitFeature(nullptr), mDepth(parent->GetDepth() + 1),
...
void EntangledForestNode::Split(EntangledForestData *data, EntangledForestSplitFeature *f,
                                EntangledForestNode **leftChild, EntangledForestNode **rightChild) {
  mOnFrontier = false;
...
ClusterIdxItr EntangledForestNode::SplitOnFeature(EntangledForestSplitFeature *f, EntangledForestData *data,
                                                  ClusterIdxItr start, ClusterIdxItr end) {
  auto i = start;
  auto j = end - 1;
  ClusterIdx swapidx;
...
void EntangledForestNode::UpdateLabelDistribution(std::vector<int> labels,
