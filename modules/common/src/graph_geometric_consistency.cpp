...
struct V4R_EXPORTS ViewD {
  ...
  bool operator<(const ViewD& j) const {
...
class V4R_EXPORTS Tomita {
  ...
  void addClique(const VectorType& clique) {
  ...
  void extend(SetType& cand, SetType& done, VectorType& clique_so_far) {
    ...
    std::set_difference(cand.begin(), cand.end(), pivot_nbrs.begin(), pivot_nbrs.end(),
                        std::inserter(small_cand, small_cand.begin()));
 ...
 public:
  ...
  void find_cliques(const Graph& G, size_t num_v) {
    SetType cand, done;
  ...
  size_t getNumCliquesFound() const {
    return cliques_found_.size();
  }

  std::vector<VectorType> getCliques() const {
    return cliques_found_;
  }
};

bool best_extended_cliques(const ExtendedClique& a, const ExtendedClique& b) {
  float a_value = a.normalized_clique_size_ * 0.25f + a.avg_descriptor_distance_ * 0.25f;
  float b_value = b.normalized_clique_size_ * 0.25f + b.avg_descriptor_distance_ * 0.25f;
...
void GraphGeometricConsistencyGrouping<PointModelT, PointSceneT>::cleanGraph(GraphGGCG& g, size_t gc_thres) {
...
void GraphGeometricConsistencyGrouping<PointModelT, PointSceneT>::clusterCorrespondences(
  ...
  if (model_scene_corrs_->empty())
    throw std::runtime_error(
        "[pcl::GeometricConsistencyGrouping::clusterCorrespondences()] Error! Correspondences not set, please set "
        "them before calling again this function.\n");
  ...
  for (size_t k = 0; k < model_scene_corrs_->size(); k++) {
    ...
    const auto& scene_normal_k = scene_normals_->points[scene_index_k].getNormalVector4fMap();
    const auto& model_normal_k = input_normals_->points[model_index_k].getNormalVector4fMap();
    ...
    for (size_t j = (k + 1); j < model_scene_corrs_->size(); ++j) {
      ...
      const auto& scene_normal_j = scene_normals_->points[scene_index_j].getNormalVector4fMap();
      const auto& model_normal_j = input_normals_->points[model_index_j].getNormalVector4fMap();
  ...
  typename boost::property_map<GraphGGCG, edge_component_t>::type components =
      get(edge_component, correspondence_graph);
  ...
  for (size_t c = 0; c < n_cc; c++) {
    ...
    if (cliques_computation_possible[c] &&
        arboricity < 25 /*&& (num_v_in_cc < 400) && (num_edges (connected_graph) < 8000) && arboricity < 10*/) {
      ...
      if (tom.getMaxTimeReached()) {
        LOG(WARNING) << "Max time ( " << std::setprecision(2) << param_.max_time_allowed_cliques_comptutation_
                     << " ms) reached during clique computation. ";
      ...
      for (size_t k = 0; k < cliques.size(); k++) {
        ...
        for (size_t jj = 0; jj < cliques[k].size(); jj++) {
          int scene_index_j = model_scene_corrs_->operator[](cliques[k][jj]).index_match;
          int model_index_j = model_scene_corrs_->operator[](cliques[k][jj]).index_query;
          const auto& scene_point_j = scene_->points[scene_index_j].getVector4fMap();
          const auto& model_point_j = input_->points[model_index_j].getVector4fMap();
          ...
          for (size_t kk = (jj + 1); kk < cliques[k].size(); kk++) {
            ...
            int scene_index_k = model_scene_corrs_->operator[](cliques[k][kk]).index_match;
            int model_index_k = model_scene_corrs_->operator[](cliques[k][kk]).index_query;

            const auto& scene_point_k = scene_->points[scene_index_k].getVector4fMap();
            const auto& model_point_k = input_->points[model_index_k].getVector4fMap();

            float dist_trg = (model_point_k - model_point_j).norm();
            float dist_ref = (scene_point_k - scene_point_j).norm();
            float distance = fabs(dist_trg - dist_ref);
            avg_3D_dist += distance;
            far_away_average_weight_ += std::min<float>((dist_ref / max_dist_), 1.f);
          }
        }
      ...
      float max_avg_3D_dist = 0.f;
      float max_avg_descriptor_dist = 0.f;
      ...
      for (size_t k = 0; k < cliques.size(); k++) {
        ...
        extended_cliques[k].avg_descriptor_distance_ =
            1.f - (extended_cliques[k].avg_descriptor_distance_ / max_avg_descriptor_dist);
        extended_cliques[k].normalized_clique_size_ =
            extended_cliques[k].correspondences_.size() / static_cast<float>(max_clique_size);
      }
      ...
      for (const std::vector<size_t> clique : cliques) {
        ...
        if (used >= param_.gc_threshold_) {
          ...
          for (size_t j = 0; j < new_clique.size(); j++) {
            CHECK(new_clique[j] < model_scene_corrs_->size());
          ...
          if ((filtered_corrs.size() >= param_.gc_threshold_) && !found && !inlier_indices.empty()) {
            ...
            if (param_.check_normals_orientation_) {
              for (size_t j = 0; j < filtered_corrs.size(); j++) {
                int model_idx = filtered_corrs[j].index_query;
                int scene_idx = filtered_corrs[j].index_match;
                if (pcl::isFinite(input_normals_->points[model_idx]) &&
                    pcl::isFinite(scene_normals_->points[scene_idx])) {
                  const auto& model_normal = input_normals_->points[model_idx].getNormalVector4fMap();
                  const auto& scene_normal = scene_normals_->points[scene_idx].getNormalVector4fMap();
                  const Eigen::Vector4f model_normal_aligned = trans * model_normal;

                  if (model_normal_aligned.dot(scene_normal) >= (1.f - param_.thres_dot_distance_))
                    all_wrong = false;
                }
              }
            }
      ...
      //            std::cout << "N edges: " << num_edges (connected_graph) << " vertices:" << num_v_in_cc << "
      //            arboricity:" << arboricity <<  std::endl;
        ...
        for (size_t j = 0; j < model_scene_corrs_->size(); j++) {
          if (j != i && !taken_corresps[j]) {
            ...
            for (size_t k = 0; k < consensus_size; k++) {
              // check if edge (j, consensus_set[k] exists in the graph, if it does not, is_a_good_candidate =
              // false!...
...
void GraphGeometricConsistencyGroupingParameter::init(boost::program_options::options_description& desc,
  ...
  desc.add_options()((section_name + ".size_thresh,c").c_str(),
  ...
  desc.add_options()((section_name + ".use_graph").c_str(), po::value<bool>(&use_graph_)->default_value(use_graph_),
                     " ");
  desc.add_options()(
      (section_name + ".dist_for_clutter_factor").c_str(),
  ...
  desc.add_options()(
      (section_name + ".max_time_for_cliques_computation").c_str(),
  ...
  desc.add_options()((section_name + ".ransac_threshold").c_str(),
                     ...
                     " ");
  desc.add_options()((section_name + ".prune").c_str(), po::value<bool>(&prune_)->default_value(prune_), " ");
