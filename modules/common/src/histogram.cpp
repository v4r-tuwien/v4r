...
void computeHistogram(const Eigen::MatrixXf &data, Eigen::MatrixXi &histogram, size_t bins, float min, float max) {
...
void shiftHistogram(const Eigen::VectorXi &hist, Eigen::VectorXi &hist_shifted, bool direction) {
  int bins = hist.rows();
...
Eigen::VectorXf specifyHistogram(const Eigen::VectorXf &input_image, const Eigen::VectorXf &desired_image, size_t bins,
                                 float min, float max) {
