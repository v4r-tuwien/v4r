...
cv::Mat cropImage(const cv::Mat &img, const cv::Rect &roi_original, size_t margin, bool make_square) {
  int width = img.cols;
  int height = img.rows;
  ...
  if (make_square) {
    if (roi.width > roi.height) {
      ...
      if (roi.height < roi.width)  // in case the roi reaches outside of image, use some pixel from the other side
                                   // instead to avoid changing aspect ratio
      ...
      if (roi.width < roi.height)  // in case the roi reaches outside of image, use some pixel from the other side
                                   // instead to avoid changing aspect ratio
