...
class NguyenNoiseModelParameter {
 public:
  bool use_depth_edges_;  ///< if true, uses PCL's organized edge detection algorithm to compute distance of each pixel
                          /// to these discontinuites.
  float focal_length_;    ///< Focal length of the camera
  NguyenNoiseModelParameter() : use_depth_edges_(true), focal_length_(525.f) {}
};
...
class V4R_EXPORTS NguyenNoiseModel {
 private:
  ...
  std::vector<std::vector<float>> pt_properties_;  ///< for each pixel save lateral [idx=0] and axial sigma [idx=1] as
                                                   /// well as Euclidean distance to depth discontinuity [idx=2]
  NguyenNoiseModelParameter param_;

 public:
  NguyenNoiseModel(const NguyenNoiseModelParameter &param = NguyenNoiseModelParameter()) : param_(param) {}
   ...
   */
  void setInputNormals(const pcl::PointCloud<pcl::Normal>::ConstPtr &normals) {
    normals_ = normals;
  }
   ...
   * discontinuity [idx=2]
   */
  std::vector<std::vector<float>> getPointProperties() const {
