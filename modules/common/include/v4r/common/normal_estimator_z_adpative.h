 ...
 * @author Andreas Richtsfeld, Johann Prankl (prankl@acin.tuwien.ac.at), Thomas Faeulhammer
 * (faeulhammer@acin.tuwien.ac.at)
 * @date 2012
 * @brief
 *
 */
...
class V4R_EXPORTS ZAdaptiveNormalsParameter {
 public:
  ...
  ZAdaptiveNormalsParameter(float radius = 0.02, int kernel = 5, bool adaptive = false, float kappa = 0.005125,
                            float d = 0.0, std::vector<int> kernel_radius = {3, 3, 3, 3, 4, 5, 6, 7})
  : radius_(radius), kernel_(kernel), adaptive_(adaptive), kappa_(kappa), d_(d), kernel_radius_(kernel_radius) {}
  ...
  std::vector<std::string> init(const std::vector<std::string> &command_line_arguments) {
    ...
    desc.add_options()("help,h", "produce help message")("normals_radius",
                                                         po::value<float>(&radius_)->default_value(radius_),
                                                         "euclidean inlier radius for normal computation.")(
        "normals_kernel_size", po::value<int>(&kernel_)->default_value(kernel_), "kernel radius in pixel.")(
        "normals_z_adaptive", po::value<bool>(&adaptive_)->default_value(adaptive_),
        "if true, adapts kernel radius with distance of point to camera.")(
...
class V4R_EXPORTS ZAdaptiveNormalsPCL : public NormalEstimator<PointT> {
 ...
 public:
  ZAdaptiveNormalsPCL(const ZAdaptiveNormalsParameter &p = ZAdaptiveNormalsParameter()) : param_(p) {
