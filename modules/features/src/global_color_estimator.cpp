...
Eigen::MatrixXf getTransformedRGB(const pcl::PointCloud<PointT> &cloud) {
...
template <>
bool GlobalColorEstimator<pcl::PointXYZ>::compute(Eigen::MatrixXf &signature) {
  LOG(FATAL) << "Given point type does not contain color information. Therefore cannot describe color features. Will "
                "return nothing.";
...
template <typename PointT>
bool GlobalColorEstimator<PointT>::compute(Eigen::MatrixXf &signature) {
