 ...
 */
ImGradientDescriptor::ImGradientDescriptor(const Parameter &p) : param(p) {
 ...
 * @param im image patch to compute the descriptor (min. 18x18 = 16x16 + 1px boarder)
 */
void ImGradientDescriptor::compute(const cv::Mat_<unsigned char> &im, const cv::Mat_<float> &weight,
