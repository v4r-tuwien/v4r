...
GlobalConcatEstimator<PointT>::GlobalConcatEstimator(std::vector<std::string> &boost_command_line_arguments,
                                                     const GlobalConcatEstimatorParameter &p)
: need_normals_(false), param_(p) {
  descr_name_ = "global";
  ...
  VLOG(1) << "Initialized global concatenated pipeline with " << descr_name_ << " resulting in " << feature_dimensions_
          << " feature dimensions.";
...
template <typename PointT>
bool GlobalConcatEstimator<PointT>::compute(Eigen::MatrixXf &signature) {
  ...
  //    CHECK(signature_esf.rows() == signature_simple_shape.rows()  && signature_simple_shape.rows() ==
  //    signature_color.rows() );

  signature =
      Eigen::MatrixXf(num_signatures, signature_esf.cols() + signature_simple_shape.cols() + signature_color.cols() +
                                          signature_ourcvfh.cols() + signature_cnn.cols());
