...
void CNN_Feat_Extractor<PointT, Dtype>::WrapInputLayer(std::vector<cv::Mat>* input_channels) {
...
template <typename PointT, typename Dtype>
void CNN_Feat_Extractor<PointT, Dtype>::Preprocess(const cv::Mat& img, std::vector<cv::Mat>* input_channels) {
...
template <typename PointT, typename Dtype>
void CNN_Feat_Extractor<PointT, Dtype>::SetMean(const std::string& mean_file) {
...
template <typename PointT, typename Dtype>
int CNN_Feat_Extractor<PointT, Dtype>::init() {
...
template <typename PointT, typename Dtype>
bool CNN_Feat_Extractor<PointT, Dtype>::compute(const cv::Mat& img, Eigen::MatrixXf& signature) {
  if (!init_) {
    init();
    init_ = true;
  }
...
template <typename PointT, typename Dtype>
bool CNN_Feat_Extractor<PointT, Dtype>::compute(Eigen::MatrixXf& signature) {
  ...
  pcl_opencv_converter.setBackgroundColor(param_.background_color_(0), param_.background_color_(1),
                                          param_.background_color_(2));
  ...
  std::cerr << "Closing operation not implemented right now!"
            << std::endl;  /// TODO: re-implement closing operation to avoid sharp transitions from noisy measurements
