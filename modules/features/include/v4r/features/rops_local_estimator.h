...
class V4R_EXPORTS ROPSLocalEstimationParameter {
 public:
  ...
  ROPSLocalEstimationParameter() : support_radius_(0.0285f), number_of_partition_bin_(5), number_of_rotations_(3) {}
...
class V4R_EXPORTS ROPSLocalEstimation : public LocalEstimator<PointT> {
 ...
 public:
  ROPSLocalEstimation(const ROPSLocalEstimationParameter &p = ROPSLocalEstimationParameter()) : param_(p) {
    descr_name_ = "rops";
    descr_type_ = FeatureType::SHOT;
    descr_dims_ = 135;
  }

  void setSupportRadius(float radius) {
