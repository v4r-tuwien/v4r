...
class V4R_EXPORTS ColorOURCVFHEstimator : public OURCVFHEstimator<PointInT, FeatureT> {
 ...
 public:
  ...
  void setUseRFForColor(bool b) {
    use_RF_ = b;
  }

  bool estimate(const PointInTPtr& in, PointInTPtr& processed,
          ...
          if (!use_RF_) {
            ...
            for (size_t t = 0; t < cvfh_signatures.points.size(); t++) {
              ...
              for (int k = 0; k < static_cast<int>(grid->points.size()); k++) {
                ...
                float wx = 1.f - std::exp(-((p[0] * p[0]) /
                                            (2.f * sigma_sq)));  // how is the weight distributed among two semi-cubes
              ...
              if (normalize_bins_) {
                for (int d = 0; d < 1327; ++d)
                  vfh_signature.points[0].histogram[d] *=
                      static_cast<float>(indices_cluster_used[t].indices.size()) / 100.f;
              }
