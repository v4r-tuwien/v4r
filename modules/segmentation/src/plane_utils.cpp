...
V4R_EXPORTS typename pcl::PointCloud<PointT>::Ptr getConvexHullCloud(
...
V4R_EXPORTS std::vector<int> get_largest_connected_plane_inliers(const pcl::PointCloud<PointT> &cloud,
                                                                 ...
                                                                 float plane_inlier_threshold, float cluster_tolerance,
...
V4R_EXPORTS void visualizePlane(const typename pcl::PointCloud<PointT>::ConstPtr &cloud, const Eigen::Vector4f &plane,
                                float inlier_threshold, const std::string &window_title) {
...
#define PCL_INSTANTIATE_get_largest_connected_plane_inliers(T)                  \
  template V4R_EXPORTS std::vector<int> get_largest_connected_plane_inliers<T>( \
      const pcl::PointCloud<T> &, const Eigen::Vector4f &, float, float, int);
PCL_INSTANTIATE(get_largest_connected_plane_inliers, PCL_XYZ_POINT_TYPES)

#define PCL_INSTANTIATE_get_largest_connected_inliers(T)                                             \
  template V4R_EXPORTS std::vector<int> get_largest_connected_inliers<T>(const pcl::PointCloud<T> &, \
                                                                         const std::vector<int> &, float, int);
PCL_INSTANTIATE(get_largest_connected_inliers, PCL_XYZ_POINT_TYPES)

#define PCL_INSTANTIATE_getConvexHullCloud(T) \
  template V4R_EXPORTS pcl::PointCloud<T>::Ptr getConvexHullCloud<T>(const pcl::PointCloud<T>::ConstPtr);
PCL_INSTANTIATE(getConvexHullCloud, PCL_XYZ_POINT_TYPES)

#define PCL_INSTANTIATE_visualizePlane(T)                                                                           \
  template V4R_EXPORTS void visualizePlane<T>(const pcl::PointCloud<T>::ConstPtr &, const Eigen::Vector4f &, float, \
                                              const std::string &);
PCL_INSTANTIATE(visualizePlane, PCL_XYZ_POINT_TYPES)

#define PCL_INSTANTIATE_visualizePlanes(T)      \
  template V4R_EXPORTS void visualizePlanes<T>( \
