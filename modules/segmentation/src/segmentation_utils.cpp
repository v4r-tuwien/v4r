...
void visualizeClusters(const typename pcl::PointCloud<PointT>::ConstPtr &cloud,
                       const std::vector<std::vector<int>> &cluster_indices, const std::string &window_title) {
  int vp1, vp2;
  ...
  for (size_t i = 0; i < cluster_indices.size(); i++) {
    ...
    *colored_cloud += cluster;
    ...
    txt << std::setfill(' ') << std::setw(7) << cluster_indices[i].size() << " pts";
...
template <typename PointT>
void visualizeCluster(const typename pcl::PointCloud<PointT>::ConstPtr &cloud, const std::vector<int> &cluster_indices,
...
#define PCL_INSTANTIATE_visualizeClusters(T)                                           \
  template V4R_EXPORTS void visualizeClusters<T>(const pcl::PointCloud<T>::ConstPtr &, \
                                                 const std::vector<std::vector<int>> &, const std::string &);
PCL_INSTANTIATE(visualizeClusters, PCL_XYZ_POINT_TYPES)

#define PCL_INSTANTIATE_visualizeCluster(T)                                                                     \
