...
void SACNormalsPlaneExtractor<PointT>::compute() {
  ...
  if (cloud_filtered->points.size() < k_) {
    PCL_WARN("[DominantPlaneSegmentation] Filtering returned %lu points! Aborting.", cloud_filtered->points.size());
    return;
  }
  ...
  Eigen::Vector4f plane =
      Eigen::Vector4f(coefficients.values[0], coefficients.values[1], coefficients.values[2], coefficients.values[3]);
