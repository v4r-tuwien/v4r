...
bool isInPlane(const Eigen::Vector4f &plane1, const Eigen::Vector3f &centerPlane2, float distThreshold) {
  float distance = fabs(dist2plane(
      centerPlane2,
      plane1));  // std::abs(1.0f-plane1.dot(centerPlane2))/plane1.norm();//DEBUG!!!!!!!!why does this get zero??????
...
template <typename PointT>
void PlaneExtractorTile<PointT>::compute() {
  if (!allocateMemory())
