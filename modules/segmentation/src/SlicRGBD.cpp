...
SlicRGBD::SlicRGBD(const Parameter &p) : param(p), num_superpixel(-1) {}
...
void SlicRGBD::drawContours(cv::Mat_<cv::Vec3b> &im_rgb, const cv::Mat_<int> &labels, int r, int g, int b) {
  ...
  for (int j = 0; j < numboundpix; j++) {
    ...
    if (have_col)
      im_rgb(ii) = cv::Vec3b(b, g, r);
    else
      im_rgb(ii) = cv::Vec3b(255, 255, 255);
    ...
    for (int n = 0; n < 8; n++) {
      ...
      if ((x >= 0 && x < width) && (y >= 0 && y < height)) {
        int ind = y * width + x;
        if (!have_col && !istaken[ind])
          im_rgb(ind) = cv::Vec3b(0, 0, 0);
      }
    }
  }
}
 ...
 * getSeeds2
 */
void SlicRGBD::getSeeds2(const pcl::PointCloud<pcl::PointXYZRGB> &cloud, const pcl::PointCloud<pcl::Normal> &normals,
                         const cv::Mat_<cv::Vec3d> &im_lab, const std::vector<bool> &valid,
                         std::vector<SlicRGBDPoint> &seeds, const int &step) {
  int numseeds(0);
  int xe, n(0);
 ...
 * getSeeds
 */
void SlicRGBD::getSeeds(const pcl::PointCloud<pcl::PointXYZRGB> &cloud, const pcl::PointCloud<pcl::Normal> &normals,
                        const cv::Mat_<cv::Vec3d> &im_lab, const std::vector<bool> &valid,
                        std::vector<SlicRGBDPoint> &seeds, const int &step) {
  int numseeds(0);
  int xe, n(0);
 ...
 */
void SlicRGBD::performSlicRGBD(const pcl::PointCloud<pcl::PointXYZRGB> &cloud,
                               const pcl::PointCloud<pcl::Normal> &normals, const cv::Mat_<cv::Vec3d> &im_lab,
                               const std::vector<bool> &valid, std::vector<SlicRGBDPoint> &seeds, cv::Mat_<int> &labels,
                               const int &step) {
  int width = im_lab.cols;
  int height = im_lab.rows;
  int sz = width * height;
      ...
      for (int y = y1; y < y2; y++) {
        for (int x = x1; x < x2; x++) {
          ...
          if (valid[idx]) {
            ...
            // cout<<dist<<" "<<dist_xy<<" "<<dist_xyz<<" "<<dist_cosa<<", "<<dist_xy*invwt_xy<<" "<<dist_xyz*wt_xyz<<"
            // "<<dist_cosa*wt_cosa<<endl;
            dist += (dist_xy * invwt_xy + dist_xyz * wt_xyz + dist_cosa * wt_cosa);

            // dist =  sqr(lab[0] - pt.l) + sqr(lab[1] - pt.a) + sqr(lab[2] - pt.b);
            // distxy =		sqr((x - pt.x)) + sqr(y - pt.y);
            //            dist = (lab[0]-pt.l)*(lab[0]-pt.l) + (lab[1]-pt.a)*(lab[1]-pt.a) +
            //            (lab[2]-pt.b)*(lab[2]-pt.b);
 ...
 */
void SlicRGBD::enforceLabelConnectivity(cv::Mat_<int> &labels, cv::Mat_<int> &out_labels, int &numlabels,
                                        const int &K) {
...
void SlicRGBD::segmentSuperpixel(cv::Mat_<int> &labels, int &numlabels) {
  ...
  const int superpixelsize =
      (num_superpixel == -1 ? param.superpixelsize
                            : 0.5 + double(cloud->width * cloud->height) / double(num_superpixel));
 ...
 * @param _normals
 */
void SlicRGBD::setCloud(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr &_cloud,
