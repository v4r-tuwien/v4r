 ...
 * @param points3d
 */
void TemporalSmoothingFilter::getPoints3D(const pcl::PointCloud<pcl::PointXYZRGB> &cloud,
 ...
 * @param points3d
 */
void TemporalSmoothingFilter::filterValidPoints3D(std::vector<cv::Point2f> &points,
                                                  std::vector<Eigen::Vector3f> &points3d) {
 ...
 * @param points3d
 */
void TemporalSmoothingFilter::filterValidPoints3D(std::vector<cv::Point2f> &pts1, std::vector<Eigen::Vector3f> &pts3d1,
                                                  std::vector<cv::Point2f> &pts2,
                                                  std::vector<Eigen::Vector3f> &pts3d2) {
 ...
 * @param points3d
 */
void TemporalSmoothingFilter::filterInliers(std::vector<cv::Point2f> &pts1, std::vector<Eigen::Vector3f> &pts3d1,
 ...
 * @param sf_pose
 */
// void TemporalSmoothingFilter::addCloud(const pcl::PointCloud<pcl::PointXYZRGB> &cloud, const Eigen::Matrix4f &pose,
// v4r::DataMatrix2D<Surfel> &sf_cloud, Eigen::Matrix4f &sf_pose)
//{
//  if (intrinsic.empty())
...
void TemporalSmoothingFilter::addCloud(const pcl::PointCloud<pcl::PointXYZRGB> &cloud, const Eigen::Matrix4f &pose,
                                       v4r::DataMatrix2D<Surfel>::Ptr &sf_cloud_, Eigen::Matrix4f &sf_pose_) {
  if (intrinsic.empty())
    throw std::runtime_error("[TemporalSmoothingFilter::addCloud] Camera parameter not set!");
    ...
    for (int v = 0; v < ref_tmp.rows; v++) {
      for (int u = 0; u < ref_tmp.cols; u++) {
        ...
        if (x >= 0 && x < sf_width && y >= 0 && y < sf_height && fabs(cloud(x, y).z - pt[2]) < thr_depth_cutoff) {
          ...
          sf.norm += norm;
          if (s.weight < param.max_integration_frames)
            sf.is_stable = false;
          else
            sf.is_stable = true;
        }
        if (x + 1 >= 0 && x + 1 < sf_width && y >= 0 && y < sf_height &&
            fabs(cloud(x + 1, y).z - pt[2]) < thr_depth_cutoff) {
        ...
        if (x >= 0 && x < sf_width && y + 1 >= 0 && y + 1 < sf_height &&
            fabs(cloud(x, y + 1).z - pt[2]) < thr_depth_cutoff) {
        ...
        if (x + 1 >= 0 && x + 1 < sf_width && y + 1 >= 0 && y + 1 < sf_height &&
            fabs(cloud(x + 1, y + 1).z - pt[2]) < thr_depth_cutoff) {
    ...
    for (unsigned v = 0; v < cloud.height; v++) {
      for (unsigned u = 0; u < cloud.width; u++) {
        ...
        if (fabs(sf.weight) <= std::numeric_limits<float>::epsilon()) {
          ...
          sf.g = pt_.g;
          sf.b = pt_.b;
          if (sf.weight < param.max_integration_frames)
            sf.weight += 1;
        }
      }
    }
 ...
 * @param out
 */
void TemporalSmoothingFilter::convertSurfelMap(const v4r::DataMatrix2D<Surfel> &cfilt,
 ...
 * @param out
 */
void TemporalSmoothingFilter::logGlobalMap(const v4r::DataMatrix2D<Surfel> &cfilt, const Eigen::Matrix4f &pose,
                                           pcl::PointCloud<pcl::PointXYZRGBNormal> &out) {
  ...
  for (int v = 0; v < sf_cloud_.rows; v++) {
    for (int u = 0; u < sf_cloud_.cols; u++) {
      ...
      for (z = 0; z < 4; z++) {
        ...
        if (u + p[0] >= 0 && u + p[0] < sf_cloud_.cols && v + p[1] >= 0 && v + p[1] < sf_cloud_.rows && u + p[2] >= 0 &&
            u + p[2] < sf_cloud_.cols && v + p[3] >= 0 && v + p[3] < sf_cloud_.rows) {
 ...
 */
void TemporalSmoothingFilter::filter(const pcl::PointCloud<pcl::PointXYZRGB> &cloud,
                                     pcl::PointCloud<pcl::PointXYZRGBNormal> &filtered_cloud, Eigen::Matrix4f &pose) {
  ...
  if (shm.prev_gray.empty()) {
    ...
    cv::calcOpticalFlowPyrLK(shm.prev_gray, shm.gray, shm.points[0], shm.points[1], status, err, param.win_size, 3,
                             param.termcrit, shm.lk_flags, 0.001);
  ...
  if (param.compute_normals)
    computeNormal(*sf_cloud);
  convertSurfelMap(*sf_cloud, filtered_cloud);
  if (param.global_map_size > 0)
    logGlobalMap(*sf_cloud, pose, *global_cloud);
