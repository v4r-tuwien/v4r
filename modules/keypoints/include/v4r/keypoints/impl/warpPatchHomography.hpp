...
inline T getInterpolated(const unsigned char *im, int rows, int cols, const T pt[2]) {
  ...
  return ((1. - ax) * (1. - ay) * *ptr_y0 + ax * (1. - ay) * ptr_y0[1] + (1. - ax) * ay * *ptr_y1 +
          ax * ay * ptr_y1[1]);
}
 ...
 */
template <typename T>
inline void mapPoint(const Eigen::Matrix<T, 2, 1> &pt_in, const Eigen::Matrix<T, 3, 3> &H,
                     Eigen::Matrix<T, 2, 1> &pt_out) {
 ...
 */
template <typename T>
bool warpPatchHomography(const cv::Mat_<unsigned char> &im, const Eigen::Matrix<T, 3, 3> &H,
 ...
 */
template <typename T>
bool warpPatchHomography(const unsigned char *im, int im_rows, int im_cols, const T H[9], unsigned char *patch,
