...
class V4R_EXPORTS ObjectView {
 public:
  ...
  ObjectView(Object *_o, int _camera_id = -1)
  : idx(-1), camera_id(_camera_id), center(Eigen::Vector3f::Zero()), object(_o) {}
  ...
  inline void addPt(const Eigen::Vector3f &pt,
                    const Eigen::Vector3f &n = Eigen::Vector3f(std::numeric_limits<double>::quiet_NaN(),
  ...
  /* add a keypoint */
  void add(const cv::KeyPoint &key, float *d, int dsize, const Eigen::Vector3f &pt3, const Eigen::Vector3f &n,
           const Eigen::Vector3f &vr,
           const Eigen::Vector3f &cam_pt = Eigen::Vector3f(std::numeric_limits<float>::quiet_NaN(),
                                                           std::numeric_limits<float>::quiet_NaN(),
                                                           std::numeric_limits<float>::quiet_NaN()),
           const int &part_idx = -1);

  /* add a keypoint */
  void add(const cv::KeyPoint &key, float *d, int dsize, unsigned glob_idx, const Eigen::Vector3f &vr,
           const Eigen::Vector3f &cam_pt = Eigen::Vector3f(std::numeric_limits<float>::quiet_NaN(),
                                                           std::numeric_limits<float>::quiet_NaN(),
                                                           std::numeric_limits<float>::quiet_NaN()),
           const int &part_idx = -1);
...
class V4R_EXPORTS Object {
 public:
  ...
  inline bool haveCodebook() {
    return !cb_centers.empty() && cb_centers.rows == (int)cb_entries.size();
  }

  /** add a new global 3d point **/
  inline unsigned addPt(const Eigen::Vector3f &pt,
                        const Eigen::Vector3f &n = Eigen::Vector3f(std::numeric_limits<double>::quiet_NaN(),
                                                                   std::numeric_limits<double>::quiet_NaN(),
                                                                   std::numeric_limits<double>::quiet_NaN())) {
  ...
  ObjectView &addObjectView(const Eigen::Matrix4f &_pose,
    ...
    if (im.empty())
      views.back()->image = cv::Mat_<unsigned char>();
    else
      im.copyTo(views.back()->image);
    return *views.back();
  }
  ...
  void initProjections(ObjectView &view) {
    if (view.keys.size() != view.points.size())
      std::runtime_error("[ObjectView::init] keys.size()!=points.size()!");
    view.projs.clear();
    view.projs.resize(view.keys.size());
    for (unsigned i = 0; i < view.keys.size(); i++) {
      view.projs[i].push_back(
          triple<int, cv::Point2f, Eigen::Vector3f>(view.camera_id, view.keys[i].pt, view.cam_points[i]));
    }
  }

  /* add projections */
  void addProjections(ObjectView &view, const std::vector<std::pair<int, cv::Point2f>> &im_pts,
                      const Eigen::Matrix4f &pose) {
    if (im_pts.size() > view.projs.size())
      std::runtime_error("[ObjectView::add] Invalid number of points! Did you call init?");
    const Eigen::Vector3f NaNs(std::numeric_limits<float>::quiet_NaN(), std::numeric_limits<float>::quiet_NaN(),
                               std::numeric_limits<float>::quiet_NaN());
    for (unsigned i = 0; i < im_pts.size(); i++)
      view.projs[im_pts[i].first].push_back(
          triple<int, cv::Point2f, Eigen::Vector3f>(cameras.size(), im_pts[i].second, NaNs));
  ...
  void addProjections(ObjectView &view, const std::vector<std::pair<int, cv::Point2f>> &im_pts, int camera_idx) {
    ...
    for (unsigned i = 0; i < im_pts.size(); i++)
      view.projs[im_pts[i].first].push_back(
          triple<int, cv::Point2f, Eigen::Vector3f>(camera_idx, im_pts[i].second, NaNs));
  }

  /* add projections */
  void addProjections(ObjectView &view, const std::vector<std::pair<int, cv::Point2f>> &im_pts,
                      const std::vector<Eigen::Vector3f> &pts3, const Eigen::Matrix4f &pose) {
    if (im_pts.size() > view.projs.size() || im_pts.size() != pts3.size())
      std::runtime_error("[ObjectView::add] Invalid number of points! Did you call init?");
    const Eigen::Vector3f NaNs(std::numeric_limits<float>::quiet_NaN(), std::numeric_limits<float>::quiet_NaN(),
                               std::numeric_limits<float>::quiet_NaN());
    for (unsigned i = 0; i < im_pts.size(); i++)
      view.projs[im_pts[i].first].push_back(
          triple<int, cv::Point2f, Eigen::Vector3f>(cameras.size(), im_pts[i].second, pts3[i]));
    cameras.push_back(pose);
  }

  /* add projections */
  void addProjections(ObjectView &view, const std::vector<std::pair<int, cv::Point2f>> &im_pts,
                      const std::vector<Eigen::Vector3f> &pts3, int camera_idx) {
    if (im_pts.size() > view.projs.size() || im_pts.size() != pts3.size())
      std::runtime_error("[ObjectView::add] Invalid number of points! Did you call init?");
    const Eigen::Vector3f NaNs(std::numeric_limits<float>::quiet_NaN(), std::numeric_limits<float>::quiet_NaN(),
                               std::numeric_limits<float>::quiet_NaN());
