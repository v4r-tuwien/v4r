...
: param(p), run(false), have_thread(false), nb_add(0), cnt_not_reliable_pose(0),
  inv_last_add_proj_pose(Eigen::Matrix4f::Identity()), last_reliable_pose(Eigen::Matrix4f::Identity()),
  loop_in_progress(false), have_loop_data(0) {
 ...
 * viewPointChange
 */
double KeyframeManagementRGBD2::viewPointChange(const Eigen::Vector3f &pt, const Eigen::Matrix4f &inv_pose1,
 ...
 * getPoints3D
 */
void KeyframeManagementRGBD2::getPoints3D(const DataMatrix2D<Eigen::Vector3f> &cloud,
 ...
 */
int KeyframeManagementRGBD2::getGlobalCorrespondences(const std::vector<unsigned> glob_indices,
                                                      const std::vector<std::pair<int, cv::Point2f>> &_im_pts,
  ...
  for (unsigned i = 0; i < _im_pts.size(); i++) {
    ...
    for (unsigned j = 0; j < keys.size(); j++) {
      if (points[j] == UINT_MAX) {
        dist = (Eigen::Map<const Eigen::Vector2f>(&im_pt.second.x) - Eigen::Map<const Eigen::Vector2f>(&keys[j].pt.x))
                   .squaredNorm();
  ...
  for (unsigned i = 0; i < _view.keys.size(); i++) {
    ...
    if (!isnan(normals[i][0]) && !isnan(pt[0])) {
      ...
      if (points[i] != UINT_MAX) {
        ...
        _view.addPt(R * pt + t, R * normals[i]);
...
bool KeyframeManagementRGBD2::closeLoops() {
  ...
  if (conf > 0.001)
    conf = kpTracker->detect(loop_image[0], loop_cloud[0], new_pose[0]);
  else
    return false;
  ...
  if (conf > 0.001)
    conf = kpTracker->detect(loop_image[1], loop_cloud[1], last_pose[1]);
  else
    return false;
...
void KeyframeManagementRGBD2::operate() {
  ...
  while (run) {
    ...
    if (nb_add != shm.nb_add) {
      if (!dbg.empty())
        cout << "[KeyframeManagementRGBD2::operate] create view!" << endl;
    ...
    if (have_data)
      have_new_view = createView(local_data, view);
...
int KeyframeManagementRGBD2::selectGuidedRandom(const Eigen::Matrix4f &pose) {
  ...
  for (unsigned i = 0; i < model->views.size(); i++) {
    ...
    angle = viewPointChange(_view.center, inv_pose2, inv_pose) + cameraRotationZ(inv_pose2, inv_pose);
 ...
 * addKeyframe
 */
void KeyframeManagementRGBD2::addKeyframe(const cv::Mat &image, const DataMatrix2D<Eigen::Vector3f> &cloud,
 ...
 * addProjections
 */
int KeyframeManagementRGBD2::addProjections(const DataMatrix2D<Eigen::Vector3f> &cloud, const Eigen::Matrix4f &pose,
                                            int view_idx, const std::vector<std::pair<int, cv::Point2f>> &_im_pts) {
  int cam_id = -1;
 ...
 * addLinkHyp
 */
int KeyframeManagementRGBD2::addLinkHyp1(const cv::Mat &image, const DataMatrix2D<Eigen::Vector3f> &cloud,
                                         const Eigen::Matrix4f &pose, int last_view_idx,
                                         const std::vector<std::pair<int, cv::Point2f>> &_im_pts, int new_view_idx) {
  int cam_id = -1;
 ...
 * getTrackingModel
 */
bool KeyframeManagementRGBD2::getTrackingModel(ObjectView &_view, Eigen::Matrix4f &view_pose,
                                               const Eigen::Matrix4f &current_pose, bool is_reliable_pose) {
  ...
  if (is_reliable_pose) {
    for (unsigned i = 0; i < model->views.size(); i++) {
      ...
      angle = viewPointChange(ov.center, inv_pose2, inv_pose1) + cameraRotationZ(inv_pose2, inv_pose1);

      if (angle < min_angle &&
          (inv_pose1.block<3, 1>(0, 3) - inv_pose2.block<3, 1>(0, 3)).squaredNorm() < sqr_max_dist_tracking_view) {
        min_angle = angle;
        idx = int(i);
      }
    }
