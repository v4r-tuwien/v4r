 ...
 */
ProjLKPoseTrackerLM::ProjLKPoseTrackerLM(const Parameter &p) : param(p) {
 ...
 * countInliers
 */
unsigned ProjLKPoseTrackerLM::countInliers(const std::vector<Eigen::Vector3d> &points,
                                           const std::vector<cv::Point2f> &_im_points, const Eigen::Matrix4d &pose) {
  unsigned cnt = 0;
 ...
 * getInliers
 */
void ProjLKPoseTrackerLM::getInliers(const std::vector<Eigen::Vector3d> &points,
                                     const std::vector<cv::Point2f> &_im_points, const Eigen::Matrix4d &pose,
 ...
 * optimizePoseLM
 */
void ProjLKPoseTrackerLM::optimizePoseLM(const std::vector<Eigen::Vector3d> &points, const std::vector<int> &pt_indices,
                                         const std::vector<cv::Point2f> &_im_points, const std::vector<int> &im_indices,
                                         Eigen::Matrix4d &pose) {
  Eigen::Matrix3d R = pose.topLeftCorner<3, 3>();
  Eigen::Vector3d t = pose.block<3, 1>(0, 3);
  ...
  if (lm_intrinsics.size() == 4)  // no distortions
  {
    for (unsigned i = 0; i < pt_indices.size(); i++) {
      ...
      problem.AddResidualBlock(new ceres::AutoDiffCostFunction<NoDistortionReprojectionError, 2, 4, 6, 3>(
                               ...
                               NULL, &lm_intrinsics[0], &pose_Rt[0], pt3);
 ...
 */
void ProjLKPoseTrackerLM::optimizePoseRobustLossLM(const std::vector<Eigen::Vector3d> &points,
                                                   const std::vector<int> &pt_indices,
                                                   const std::vector<cv::Point2f> &_im_points,
                                                   const std::vector<int> &im_indices, Eigen::Matrix4d &pose) {
  Eigen::Matrix3d R = pose.topLeftCorner<3, 3>();
  Eigen::Vector3d t = pose.block<3, 1>(0, 3);
  Eigen::Matrix<double, 6, 1> pose_Rt;
  ceres::RotationMatrixToAngleAxis(&R(0, 0), &pose_Rt(0));
  ...
  if (lm_intrinsics.size() == 4)  // no distortions
  {
    for (unsigned i = 0; i < pt_indices.size(); i++) {
      ...
      problem.AddResidualBlock(new ceres::AutoDiffCostFunction<NoDistortionReprojectionError, 2, 4, 6, 3>(
                                   new NoDistortionReprojectionError(im_pt.x, im_pt.y)),
                               new ceres::CauchyLoss(param.loss_scale), &lm_intrinsics[0], &pose_Rt[0], pt3);
      problem.SetParameterBlockConstant(pt3);
    }
  } else if (lm_intrinsics.size() == 9)  // radial distortions
  {
 ...
 * ransacPoseLM
 */
void ProjLKPoseTrackerLM::ransacPoseLM(const std::vector<Eigen::Vector3d> &points,
                                       const std::vector<cv::Point2f> &_im_points, Eigen::Matrix4d &pose,
                                       std::vector<int> &_inliers) {
  int k = 0;
  ...
  if (param.use_robust_loss)
    ...
    optimizePoseLM(points, _inliers, _im_points, _inliers, pose);
  getInliers(points, _im_points, pose, _inliers);
}
...
double ProjLKPoseTrackerLM::detect(const cv::Mat &image, Eigen::Matrix4f &pose) {
  ...
  for (unsigned z = 0; z < im_points.size(); z++) {
    if (converged[z] == 1) {
      ...
      if (!dbg.empty())
        cv::line(dbg, model->keys[z].pt, im_points[z], CV_RGB(255, 0, 0));
    }
  }
  ...
  if (param.use_ransac) {
    ...
    if (param.use_robust_loss)
      ...
      optimizePoseLM(model_pts, indices, query_pts, indices, pose_64f);

    getInliers(model_pts, query_pts, pose_64f, lm_inliers);
  }

  if (int(lm_inliers.size()) < 4)
    return 0.;

  for (unsigned i = 0; i < lm_inliers.size(); i++) {
    inliers.push_back(lk_inliers[lm_inliers[i]]);
    if (!dbg.empty())
      cv::circle(dbg, im_points[inliers.back()], 2, CV_RGB(0, 255, 0));
  }
