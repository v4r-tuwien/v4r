 ...
 */
KeypointPoseDetectorRT::KeypointPoseDetectorRT(const Parameter &p, const v4r::FeatureDetector::Ptr &_detector,
 ...
 * detect
 */
double KeypointPoseDetectorRT::detect(const cv::Mat &image, const DataMatrix2D<Eigen::Vector3f> &cloud,
                                      Eigen::Matrix4f &pose) {
  if (model.get() == 0)
      ...
      if (ma0.distance / matches[z][1].distance < param.nnr) {
        ...
        if (!isnan(pt[0])) {
          ...
          if (param.compute_global_pose) {
            if (ma0.trainIdx >= (int)model->points.size()) {
              query_pts.pop_back();
              continue;
            }
            model_pts.push_back(model->getPt(ma0.trainIdx).pt.cast<float>());
          } else {
            if (ma0.trainIdx >= (int)model->cam_points.size()) {
              query_pts.pop_back();
              continue;
            }
