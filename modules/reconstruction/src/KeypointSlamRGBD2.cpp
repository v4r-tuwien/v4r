...
: param(p), view_pose(Eigen::Matrix4f::Identity()), delta_pose(Eigen::Matrix4f::Identity()), conf(0.), conf_cnt(0),
  pose(Eigen::Matrix4f::Identity()), new_kf_1st_frame(-1), new_kf_2nd_frame(-1) {
 ...
 * viewPointChange
 */
double KeypointSlamRGBD2::viewPointChange(const Eigen::Vector3f &pt, const Eigen::Matrix4f &inv_pose1,
 ...
 * addKeyframe
 */
bool KeypointSlamRGBD2::addKeyframe(const ObjectView &_view, const Eigen::Matrix4f &_delta_pose,
                                    const Eigen::Matrix4f &_view_pose, const Eigen::Matrix4f &_pose, int width,
                                    int height, double _conf) {
  bool add_kf = false;
 ...
 */
// int cnt_reinit=0;
bool KeypointSlamRGBD2::track(const cv::Mat &image, const DataMatrix2D<Eigen::Vector3f> &cloud,
  ...
  if (addKeyframe(*view, delta_pose, view_pose, pose, im_gray.cols, im_gray.rows, conf)) {
    ...
    om->addKeyframe(im_gray, cloud, (view->points.size() < 4 ? Eigen::Matrix4f::Identity() : pose), tracked_view_idx,
                    im_pts);
  }
  ...
  if (conf_cnt > param.min_conf_cnt) {
    ...
    if (new_kf_1st_frame != -1)
      ...
      cam_id = om->addProjections(cloud, pose, tracked_view_idx, im_pts);
  }
