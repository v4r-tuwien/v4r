...
: width(0), height(0), sf_timestamp(0), sf_pose(Eigen::Matrix4f::Identity()), sf_nb_frames(0), run(false),
  have_thread(false) {
  setParameter(p);
}
 ...
 * @param pose
 */
void TSFilterCloudsXYZRGB::integrateDataRGB(const pcl::PointCloud<pcl::PointXYZRGB> &cloud,
 ...
 * @param pose
 */
void TSFilterCloudsXYZRGB::integrateDataRGBbilinear(const pcl::PointCloud<pcl::PointXYZRGB> &cloud,
                                                    const Eigen::Matrix4f &pose) {
  int x, y;
  cv::Point2f im_pt;
  float ax, ay;
 ...
 * @param pose
 */
void TSFilterCloudsXYZRGB::integrateDataRGBbilinear2(const pcl::PointCloud<pcl::PointXYZRGB> &cloud,
                                                     const Eigen::Matrix4f &pose) {
  int x, y;
  cv::Point2f im_pt;
  float ax, ay, w;
 ...
 * @param pose
 */
void TSFilterCloudsXYZRGB::integrateDatabilinear(const pcl::PointCloud<pcl::PointXYZRGB> &cloud,
                                                 const Eigen::Matrix4f &pose) {
  int x, y;
  cv::Point2f im_pt;
  float ax, ay, w;
 ...
 * @param pose
 */
void TSFilterCloudsXYZRGB::projectedColourTransfere(const v4r::DataMatrix2D<v4r::Surfel> &_sf_cloud,
 ...
 * @param have_track
 */
void TSFilterCloudsXYZRGB::addCloud(const pcl::PointCloud<pcl::PointXYZRGB> &cloud, const Eigen::Matrix4f &pose,
                                    const double &_timestamp, bool have_track) {
  if (!isStarted())
    start();
  ...
  for (int v = 0; v < sf_cloud.rows; v++) {
    for (int u = 0; u < sf_cloud.cols; u++) {
      ...
      for (z = 0; z < 4; z++) {
        ...
        if (u + p[0] >= 0 && u + p[0] < sf_cloud.cols && v + p[1] >= 0 && v + p[1] < sf_cloud.rows && u + p[2] >= 0 &&
            u + p[2] < sf_cloud.cols && v + p[3] >= 0 && v + p[3] < sf_cloud.rows) {
      ...
      if (z < 4) {
        ...
        s1->n = Eigen::Vector3f(std::numeric_limits<float>::quiet_NaN(), std::numeric_limits<float>::quiet_NaN(),
                                std::numeric_limits<float>::quiet_NaN());
    }
  }
}

int TSFilterCloudsXYZRGB::getFilteredCloudNormals(pcl::PointCloud<pcl::PointXYZRGBNormal> &cloud, Eigen::Matrix4f &pose,
                                                  double &timestamp) {
  int nb;
  mtx_shm.lock();
...
int TSFilterCloudsXYZRGB::getFilteredCloud(pcl::PointCloud<pcl::PointXYZRGB> &cloud, Eigen::Matrix4f &pose,
                                           double &timestamp) {
  int nb;
  mtx_shm.lock();
  cloud.resize(sf_cloud.data.size());
