...
class ColourCorrespondenceError {
 public:
  ColourCorrespondenceError(const cv::Vec3f &_col0, const cv::Vec3f &_col1) : col0(_col0), col1(_col1) {}

  /**
   */
  template <typename T>
  bool operator()(const T *const col_scale0, const T *const col_scale1, T *residuals) const {
...
class ColourScaleNormError {
 public:
  ColourScaleNormError(const int &_nb_scales,
                       const std::vector<std::vector<v4r::triple<cv::Vec3f, int, cv::Vec3f>>> &_rgb_correspondences)
 ...
 */
void OptimizeBundleColours::getLinkedFrames(
    const unsigned &idx, const std::vector<std::vector<v4r::triple<int, cv::Point2f, Eigen::Vector3f>>> &projections,
    std::set<int> &links) {
  links.clear();
 ...
 * @return
 */
bool OptimizeBundleColours::pickCol(const Eigen::Vector3f &pt, const Eigen::Matrix4f &delta_pose, const cv::Mat &im_lin,
                                    cv::Point2f &_im_pt, cv::Vec3f &col) {
  ...
  if (dist_coeffs.empty())
    ...
    projectPointToImage(&pt1[0], &intrinsic(0, 0), &dist_coeffs(0, 0), &im_pt.x);
 ...
 * @return
 */
bool OptimizeBundleColours::pickCol(const Eigen::Vector3f &pt, const Eigen::Matrix4f &delta_pose,
                                    const v4r::DataMatrix2D<Surfel> &sf_cloud, const cv::Mat &im_lin,
                                    cv::Point2f &_im_pt, cv::Vec3f &col) {
  Eigen::Vector3f pt1 = delta_pose.topLeftCorner<3, 3>() * pt + delta_pose.block<3, 1>(0, 3);
  cv::Point2f im_pt;
  ...
  if ((int)im_pt.x >= 0 && (int)im_pt.x < im_lin.cols && (int)im_pt.y >= 0 && (int)im_pt.y < im_lin.rows &&
      !isnan(sf_cloud(im_pt.y, im_pt.x).pt[0]) &&
      fabs(1. / pt[2] - 1. / sf_cloud(im_pt.y, im_pt.x).pt[2]) < param.inv_inl_dist) {
    col = im_lin.at<cv::Vec3f>(im_pt.y, im_pt.x);
    return true;
  }
  return false;
...
// double sum=0;
// unsigned nb = 0;
void OptimizeBundleColours::sampelColourCorrespondences(const std::vector<TSFFrame::Ptr> &map,
                                                        const std::vector<cv::Mat> &_lin_images, int idx0, int idx1,
                                                        std::vector<v4r::triple<cv::Vec3f, int, cv::Vec3f>> &corrs) {
  ...
  while (z < param.max_sampling_trials && cnt < param.nb_col_samples_per_linked_frame) {
    ...
    if (x >= 0 && x < frame0.sf_cloud.cols && y >= 0 && y < frame0.sf_cloud.rows && mask(y, x)) {
      ...
      if (pickCol(s0.pt, frame0.delta_cloud_rgb_pose, im_lin0, im_pt0, col0) &&
          pickCol(s0.pt, pose_im01, frame1.sf_cloud, im_lin1, im_pt1, col1)) {
        const Surfel &s1 = frame1.sf_cloud(im_pt1.y, im_pt1.x);

        if (s0.r > param.saturation_offs_low && s0.r < param.saturation_offs_high && s0.g > param.saturation_offs_low &&
            s0.g < param.saturation_offs_high && s0.b > param.saturation_offs_low &&
            s0.b < param.saturation_offs_high && s1.r > param.saturation_offs_low &&
            s1.r < param.saturation_offs_high && s1.g > param.saturation_offs_low &&
            s1.g < param.saturation_offs_high && s1.b > param.saturation_offs_low &&
            s1.b < param.saturation_offs_high) {
 ...
 */
void OptimizeBundleColours::sampleCorrespondences(const std::vector<TSFFrame::Ptr> &map,
                                                  const std::vector<cv::Mat> &_lin_images) {
  std::set<int> links;
...
void OptimizeBundleColours::optimizeColourScales(int idx) {
  ...
  //  ceres::DynamicAutoDiffCostFunction<ColourScaleNormError, stride>* cost_function = new
  //  ceres::DynamicAutoDiffCostFunction<ColourScaleNormError, stride>(new ColourScaleNormError(nb_scales,
  //  rgb_correspondences)); cost_function->AddParameterBlock(nb_scales); cost_function->SetNumResiduals(3*nb_residuals
  //  + 1);

  //  std::vector<double*> blocks;
  //  blocks.push_back(&colour_scales_data[0]);
  //  problem.AddResidualBlock(cost_function, (param.use_robust_loss?new ceres::CauchyLoss(param.loss_scale):NULL),
  //  blocks);
...
void OptimizeBundleColours::optimize(const std::vector<TSFFrame::Ptr> &map) {
  ...
  if (path_to_crf_file.size() == 0) {
    cout << "[OptimizeBundleColours::optimize] No camera response function calibration file available (crf-file)"
         << endl;
    return;
  }
  ...
  for (unsigned i = 0; i < lin_images.size(); i++) {
    ...
    cout << "i=" << i << ": " << colour_scales_data[i * 3] << " " << colour_scales_data[i * 3 + 1] << " "
         << colour_scales_data[i * 3 + 2] << " " << endl;
  }
}
