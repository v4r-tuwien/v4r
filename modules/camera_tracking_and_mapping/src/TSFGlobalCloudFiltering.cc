    ...
    for (unsigned j = 0; j < frames.size(); j++) {
      ...
      for (int v = 0; v < frame_j.rows; v++) {
        for (int u = 0; u < frame_j.cols; u++) {
          ...
          if (isnan(s_j.n[0]) || isnan(s_j.pt[0]) || isnan(s_j.n[1]) || isnan(s_j.pt[1]) || isnan(s_j.n[2]) ||
              isnan(s_j.pt[2]))
            continue;
 ...
 * @param cloud
 */
void TSFGlobalCloudFiltering::getMaxPoints(const std::vector<TSFFrame::Ptr> &frames,
 ...
 * @param frames
 */
void TSFGlobalCloudFiltering::filter(const std::vector<TSFFrame::Ptr> &frames,
                                     pcl::PointCloud<pcl::PointXYZRGBNormal> &cloud) {
