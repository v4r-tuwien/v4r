 ...
 */
TSFOptimizeBundle::TSFOptimizeBundle(const Parameter &p) : const_all_intrinsics(true) {
  setParameter(p);
}
...
void TSFOptimizeBundle::convertPosesFromRtRGB(std::vector<TSFFrame::Ptr> &map) {
  ...
  if (map.size() == poses_Rt_RGB.size()) {
    ...
    for (unsigned i = 0; i < poses_Rt_RGB.size(); i++) {
      pose_rgb = Eigen::Matrix4f::Identity();
    ...
    for (unsigned i = 0; i < map.size(); i++) {
      map[i]->delta_cloud_rgb_pose = Eigen::Matrix4f::Identity();
...
void TSFOptimizeBundle::optimizePoses(std::vector<TSFFrame::Ptr> &map) {
        ...
        for (unsigned k = 0; k < frame.projections[j].size(); k++) {
          ...
          problem.AddResidualBlock(
              new ceres::AutoDiffCostFunction<ReprojectionErrorGlobalPoseCamViewData, 2, 4, 6, 6, 3>(
                  new ReprojectionErrorGlobalPoseCamViewData(im_pt.x, im_pt.y, param.px_error_scale)),
              (param.use_robust_loss ? new ceres::CauchyLoss(param.loss_scale) : NULL), &lm_intrinsics[0], cam0, cam1,
              &pt3[0]);
          const Eigen::Vector3f &pt3v = frame.projections[j][k].third;
          if (!isnan(pt3v[0]) && !isnan(pt3v[1]) && !isnan(pt3v[2])) {
            problem.AddResidualBlock(new ceres::AutoDiffCostFunction<PointToPlaneErrorGlobalPoseCamViewData, 3, 6, 6>(
                                         new PointToPlaneErrorGlobalPoseCamViewData(
                                             pt3, n0.cast<double>(), pt3v.cast<double>(), param.depth_error_scale)),
                                     (param.use_robust_loss ? new ceres::CauchyLoss(param.loss_scale) : NULL), cam0,
                                     cam1);
          }
        }
        ...
        for (unsigned k = 0; k < frame.projections[j].size(); k++) {
          ...
          problem.AddResidualBlock(
              new ceres::AutoDiffCostFunction<RadialDistortionReprojectionErrorGlobalPoseCamViewData, 2, 9, 6, 6, 3>(
                  new RadialDistortionReprojectionErrorGlobalPoseCamViewData(im_pt.x, im_pt.y, param.px_error_scale)),
              (param.use_robust_loss ? new ceres::CauchyLoss(param.loss_scale) : NULL), &lm_intrinsics[0], cam0, cam1,
              &pt3[0]);
          const Eigen::Vector3f &pt3v = frame.projections[j][k].third;
          if (!isnan(pt3v[0]) && !isnan(pt3v[1]) && !isnan(pt3v[2]) && !isnan(n0[0]) && !isnan(n0[1]) &&
              !isnan(n0[2])) {
            problem.AddResidualBlock(new ceres::AutoDiffCostFunction<PointToPlaneErrorGlobalPoseCamViewData, 3, 6, 6>(
                                         new PointToPlaneErrorGlobalPoseCamViewData(
                                             pt3, n0.cast<double>(), pt3v.cast<double>(), param.depth_error_scale)),
  ...
  if (const_all_intrinsics) {
    ...
    if (const_intrinsics.size() > 0) {
      cout << "const nb=" << const_intrinsics.size() << endl;
      ceres::SubsetParameterization *subset_parameterization = new ceres::SubsetParameterization(9, const_intrinsics);
      ...
      for (unsigned j = 0; j < frame.points3d.size(); j++) {
        ...
        for (unsigned k = 0; k < frame.projections[j].size(); k++) {
          ...
          problem.AddResidualBlock(
              ...
              (param.use_robust_loss ? new ceres::CauchyLoss(param.loss_scale) : NULL), &lm_intrinsics[0], cam0RGB,
              cam1RGB, &pt3RGB[0]);
          const Eigen::Vector3f &pt3v = frame.projections[j][k].third;
          if (!isnan(pt3v[0]) && !isnan(pt3v[1]) && !isnan(pt3v[2]) && !isnan(n0pc[0]) && !isnan(n0pc[1]) &&
              !isnan(n0pc[2])) {
            problem.AddResidualBlock(
                new ceres::AutoDiffCostFunction<PointToPlaneErrorGlobalPoseCamViewData, 3, 6, 6>(
                    new PointToPlaneErrorGlobalPoseCamViewData(pt3pc.cast<double>(), n0pc.cast<double>(),
                                                               pt3v.cast<double>(), param.depth_error_scale)),
          ...
          if (!isnan(n0pc[0]) && !isnan(n0pc[1]) && !isnan(n0pc[2])) {
            problem.AddResidualBlock(
                new ceres::AutoDiffCostFunction<PointToPlaneErrorGlobalPoseCamViewDataOptiPt1, 3, 6, 6, 3>(
                    new PointToPlaneErrorGlobalPoseCamViewDataOptiPt1(pt3pc.cast<double>(), n0pc.cast<double>(),
                                                                      param.depth_error_scale)),
      ...
      for (unsigned j = 0; j < frame.points3d.size(); j++) {
        ...
        for (unsigned k = 0; k < frame.projections[j].size(); k++) {
          ...
          problem.AddResidualBlock(
              ...
              (param.use_robust_loss ? new ceres::CauchyLoss(param.loss_scale) : NULL), &lm_intrinsics[0], cam0pc,
              cam1pc, &delta_pose[0], &pt3RGB[0]);
          const Eigen::Vector3f &pt3v = frame.projections[j][k].third;
          if (!isnan(pt3v[0]) && !isnan(pt3v[1]) && !isnan(pt3v[2]) && !isnan(n0pc[0]) && !isnan(n0pc[1]) &&
              !isnan(n0pc[2])) {
            problem.AddResidualBlock(
          ...
          if (!isnan(n0pc[0]) && !isnan(n0pc[1]) && !isnan(n0pc[2])) {
            problem.AddResidualBlock(
                new ceres::AutoDiffCostFunction<PointToPlaneErrorGlobalPoseDeltaPoseCamViewDataOptiPt1, 3, 6, 3>(
                    new PointToPlaneErrorGlobalPoseDeltaPoseCamViewDataOptiPt1(
                        pt3pc.cast<double>(), n0pc.cast<double>(), param.depth_error_scale)),
    ...
    for (unsigned i = 0; i < map.size(); i++) {
      ...
      for (unsigned j = 0; j < frame.points3d.size(); j++) {
        ...
        for (unsigned k = 0; k < frame.projections[j].size(); k++) {
          ...
          double *cam1pc = &poses_Rt[frame.projections[j][k].first][0];
          problem.AddResidualBlock(
              new ceres::AutoDiffCostFunction<RadialDistortionReprojectionErrorGlobalPoseDeltaPoseCamViewData, 2, 9, 6,
                                              6, 6, 3>(
...
void TSFOptimizeBundle::setCameraParameterConst() {
  ...
  if (lm_intrinsics.size() == const_intrinsics.size())
    const_all_intrinsics = true;
}
...
void TSFOptimizeBundle::optimize(std::vector<TSFFrame::Ptr> &map) {
  ...
  double depth_error_scale = param.depth_error_scale;
  if (param.optimize_delta_cloud_rgb_pose_global || param.optimize_delta_cloud_rgb_pose)
    param.depth_error_scale = 0.01;

  optimizePoses(map);

  param.depth_error_scale = depth_error_scale;
  const_all_intrinsics = true;
 ...
 * @param _dist_coeffs
 */
void TSFOptimizeBundle::getCameraParameter(cv::Mat &_intrinsic, cv::Mat &_dist_coeffs) const {
  _dist_coeffs = cv::Mat();
  _intrinsic = cv::Mat();
  if (lm_intrinsics.size() >= 4) {
    _intrinsic = cv::Mat_<double>::eye(3, 3);
  ...
  if (lm_intrinsics.size() == 9) {
    ...
    _dist_coeffs.at<double>(0, 4) = lm_intrinsics[6];
