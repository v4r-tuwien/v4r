 ...
 */
TSFPoseTrackerKLT::TSFPoseTrackerKLT(const Parameter &p) : param(p), run(false), have_thread(false), data(NULL) {
 ...
 * @param points3d
 */
void TSFPoseTrackerKLT::getPoints3D(const pcl::PointCloud<pcl::PointXYZRGB> &cloud,
                                    const std::vector<cv::Point2f> &points, std::vector<Eigen::Vector3f> &points3d) {
  if (intrinsic.empty())
 ...
 * @param points3d
 */
void TSFPoseTrackerKLT::filterValidPoints3D(std::vector<cv::Point2f> &pts1, std::vector<Eigen::Vector3f> &pts3d1,
 ...
 * @param points3d
 */
void TSFPoseTrackerKLT::filterInliers(std::vector<cv::Point2f> &pts1, std::vector<Eigen::Vector3f> &pts3d1,
                                      std::vector<cv::Point2f> &pts2, std::vector<Eigen::Vector3f> &pts3d2,
 ...
 * @param converged
 */
void TSFPoseTrackerKLT::filterConverged(std::vector<cv::Point2f> &pts1, std::vector<Eigen::Vector3f> &pts3d1,
                                        std::vector<cv::Point2f> &pts2, std::vector<Eigen::Vector3f> &pts3d2,
                                        std::vector<int> &converged) {
  if (pts1.size() != pts3d1.size() || pts1.size() != pts3d2.size() || pts1.size() != pts2.size() ||
      pts1.size() != converged.size())
    return;
...
bool TSFPoseTrackerKLT::trackCamera(double &conf_ransac_iter, double &conf_tracked_points) {
  ...
  cv::calcOpticalFlowPyrLK(data->prev_gray, data->gray, data->points[0], data->points[1], status, err, param.win_size,
                           3, param.termcrit, data->lk_flags, 0.001);
  ...
  if (data->points3d[1].size() > 4) {
    ...
    for (unsigned j = 0; j < data->points3d[1].size(); j++)
      depth[j] = data->points3d[1][j][2];
    ...
    conf_tracked_points = (inliers.size() >= param.conf_tracked_points_norm
                               ? 1.
                               : ((double)inliers.size()) / (double)param.conf_tracked_points_norm);
