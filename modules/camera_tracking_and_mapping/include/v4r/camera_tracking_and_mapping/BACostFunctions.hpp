...
struct ReprojectionErrorGlobalPoseCamViewData {
  ReprojectionErrorGlobalPoseCamViewData(const double& _observed_x, const double& _observed_y,
                                         const double& _error_weight = 1.)
  : observed_x(_observed_x), observed_y(_observed_y), error_weight(_error_weight) {}
  ...
  bool operator()(const T* const intrinsics,
    ...
    // The error is the difference between the predicted and observed position.
    residuals[0] = T(error_weight) * (predicted_x - T(observed_x));
    residuals[1] = T(error_weight) * (predicted_y - T(observed_y));
...
struct PointToPlaneErrorGlobalPoseCamViewData {
  PointToPlaneErrorGlobalPoseCamViewData(const Eigen::Vector3d& _pt0, const Eigen::Vector3d& _n0,
                                         const Eigen::Vector3d& _pt1, const double& _w)
...
struct PointToPlaneErrorGlobalPoseCamViewDataOptiPt1 {
  PointToPlaneErrorGlobalPoseCamViewDataOptiPt1(const Eigen::Vector3d& _pt0, const Eigen::Vector3d& _n0,
                                                const double& _w)
  ...
  bool operator()(const T* const Rt0,  // Rotation denoted by angle axis followed with translation (...to keyframe)
                  const T* const Rt1,  // ..to proj. frame
                  const T* const X1, T* residuals) const {
    Eigen::Matrix<T, 3, 1> pt0e, n0e, pt0g, n0g, pt1t, n1t, pt1e;
...
struct PointToPlaneErrorGlobalPoseDeltaPoseCamViewDataOptiPt1 {
  PointToPlaneErrorGlobalPoseDeltaPoseCamViewDataOptiPt1(const Eigen::Vector3d& _pt0, const Eigen::Vector3d& _n0,
                                                         const double& _w)
  : pt0(_pt0), n0(_n0), error_weight(_w) {}
