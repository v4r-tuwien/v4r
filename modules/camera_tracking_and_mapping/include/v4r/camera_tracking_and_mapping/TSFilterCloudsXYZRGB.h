...
class V4R_EXPORTS TSFilterCloudsXYZRGB {
 public:
  ...
  class Parameter {
   public:
    ...
    int type;  // interpolation/ mean value method: 0..original rgb, 1..mean value, (no interpolation), 2..bilin.
               // interpolation, 3..bilin. inv_thr depth, bilin col
 ...
 private:
  ...
  void projectedColourTransfere(const v4r::DataMatrix2D<v4r::Surfel> &sf_cloud,
                                const pcl::PointCloud<pcl::PointXYZRGB> &cloud, const Eigen::Matrix4f &pose);
 ...
 public:
  ...
  void addCloud(const pcl::PointCloud<pcl::PointXYZRGB> &cloud, const Eigen::Matrix4f &pose, const double &timestamp,
                bool have_track);

  inline void lock() {
    mtx_shm.lock();
  }
  inline const double &getFiltTimestamp() {
    return sf_timestamp;
  }
  inline const Eigen::Matrix4f &getFiltPose() {
    return sf_pose;
  }
  inline const v4r::DataMatrix2D<v4r::Surfel> &getFiltCloud() {
    return sf_cloud;
  }
  inline void unlock() {
    mtx_shm.unlock();
...
inline void TSFilterCloudsXYZRGB::insertWeightedXYZRGB(float &d, cv::Vec3f &col, float &w, const pcl::PointXYZRGB &pt,
                                                       const float &w_pt) {
  if (w > 0) {
...
inline void TSFilterCloudsXYZRGB::insertWeightedRGB(cv::Vec3f &col, float &w, const pcl::PointXYZRGB &pt,
                                                    const float &w_pt) {
  col[0] = (w * col[0] + w_pt * pt.b);
  col[1] = (w * col[1] + w_pt * pt.g);
  col[2] = (w * col[2] + w_pt * pt.r);
...
inline bool TSFilterCloudsXYZRGB::getInterpolatedRGB(const pcl::PointCloud<pcl::PointXYZRGB> &cloud,
                                                     const cv::Point2f &pt, cv::Vec3f &rgb) {
  if (pt.x >= 0 && pt.y >= 0 && pt.x < cloud.width - 1 && pt.y < cloud.height - 1) {
    int xt = (int)pt.x;
    int yt = (int)pt.y;
    float ax = pt.x - xt;
    float ay = pt.y - yt;
    rgb[0] = ((1. - ax) * (1. - ay) * cloud(xt, yt).r + ax * (1. - ay) * cloud(xt + 1, yt).r +
              (1. - ax) * ay * cloud(xt, yt + 1).r + ax * ay * cloud(xt + 1, yt + 1).r);
    rgb[1] = ((1. - ax) * (1. - ay) * cloud(xt, yt).g + ax * (1. - ay) * cloud(xt + 1, yt).g +
              (1. - ax) * ay * cloud(xt, yt + 1).g + ax * ay * cloud(xt + 1, yt + 1).g);
    rgb[2] = ((1. - ax) * (1. - ay) * cloud(xt, yt).b + ax * (1. - ay) * cloud(xt + 1, yt).b +
              (1. - ax) * ay * cloud(xt, yt + 1).b + ax * ay * cloud(xt + 1, yt + 1).b);
    return true;
