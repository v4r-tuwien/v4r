...
class V4R_EXPORTS TSFGlobalCloudFilteringSimple {
 public:
  ...
  class Parameter {
   public:
    ...
    Parameter()
    : voxel_size(0.001), thr_weight(10), thr_delta_angle(75), poisson_depth(7), samples_per_node(2), crop_mesh(true),
      erosion_size(7), filter_largest_cluster(false) {}
  };
 ...
 private:
  ...
  void filterCluster(const pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr &cloud,
 ...
 public:
  ...
  void getGlobalCloudMasked(const std::vector<v4r::TSFFrame::Ptr> &frames,
                            const std::vector<cv::Mat_<unsigned char>> &masks,
                            pcl::PointCloud<pcl::PointXYZRGBNormal> &cloud);
  void getGlobalCloudFiltered(const std::vector<v4r::TSFFrame::Ptr> &frames,
                              pcl::PointCloud<pcl::PointXYZRGBNormal> &cloud);
  void getMesh(const pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr &cloud, pcl::PolygonMesh &mesh);
  //  void textureMapping(const std::vector< v4r::TSFFrame::Ptr > &frames, const pcl::PolygonMesh &mesh, const
  //  std::string &dir, pcl::TextureMesh &tex_mesh);
 ...
 * @param b
 */
inline void TSFGlobalCloudFilteringSimple::getInterpolatedRGB(const v4r::DataMatrix2D<v4r::Surfel> &cloud,
                                                              const cv::Point2f &pt, unsigned char &r, unsigned char &g,
                                                              unsigned char &b) {
  if (pt.x < cloud.cols - 1 && pt.y < cloud.rows - 1) {
    ...
    float ax = pt.x - xt;
    float ay = pt.y - yt;
    r = (unsigned char)((1. - ax) * (1. - ay) * cloud(yt, xt).r + ax * (1. - ay) * cloud(yt, xt + 1).r +
                        (1. - ax) * ay * cloud(yt + 1, xt).r + ax * ay * cloud(yt + 1, xt + 1).r);
    g = (unsigned char)((1. - ax) * (1. - ay) * cloud(yt, xt).g + ax * (1. - ay) * cloud(yt, xt + 1).g +
                        (1. - ax) * ay * cloud(yt + 1, xt).g + ax * ay * cloud(yt + 1, xt + 1).g);
    b = (unsigned char)((1. - ax) * (1. - ay) * cloud(yt, xt).b + ax * (1. - ay) * cloud(yt, xt + 1).b +
                        (1. - ax) * ay * cloud(yt + 1, xt).b + ax * ay * cloud(yt + 1, xt + 1).b);
