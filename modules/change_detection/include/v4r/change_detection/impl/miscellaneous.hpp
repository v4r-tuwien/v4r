...
typename pcl::PointCloud<PointType>::Ptr downsampleCloud(typename pcl::PointCloud<PointType>::Ptr input,
                                                         double resolution) {
...
Eigen::Affine3f resetViewpoint(typename pcl::PointCloud<PointType>::Ptr input) {
  Eigen::Vector3f offset(input->sensor_origin_(0), input->sensor_origin_(1), input->sensor_origin_(2));
