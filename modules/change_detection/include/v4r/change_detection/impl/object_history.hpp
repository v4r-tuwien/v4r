...
void ObjectsHistory<PointType>::add(const std::vector<ObjectDetection<PointType>> &detections) {
  for (typename std::vector<ObjectDetection<PointType>>::const_iterator d = detections.begin(); d < detections.end();
       d++) {
...
template <class PointType>
std::vector<ObjectChangeForVisual<PointType>> ObjectsHistory<PointType>::getChanges(ObjectState::EventT changeType) {
  std::vector<ObjectChangeForVisual<PointType>> changes;

  for (typename Db::iterator entry = db.begin(); entry != db.end(); entry++) {
    if (entry->second.getLastEvent() == changeType && entry->second.getTimeStamp() == time) {
