...
class V4R_EXPORTS Visualizer3D {
 public:
  ...
  Visualizer3D& addPointCloud(const pcl::PointCloud<PointT> cloud,
    ...
    for (typename pcl::PointCloud<PointT>::const_iterator pt = cloud_transformed.begin(); pt < cloud_transformed.end();
         pt++) {
  ...
  Visualizer3D& addColorPointCloud(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,
                                   const Eigen::Matrix4f& transformation = Eigen::Matrix4f::Identity()) {
  ...
  Visualizer3D& addPointClouds(const std::vector<pcl::PointCloud<PointT>>& clouds) {
    for (typename std::vector<pcl::PointCloud<PointT>>::const_iterator cloud = clouds.begin(); cloud < clouds.end();
         cloud++) {
      addPointCloud(*cloud);
    }
    return *this;
  ...
  Visualizer3D& addObjects(const std::vector<ObjectDetection<PointT>>& objects) {
    for (typename std::vector<ObjectDetection<PointT>>::const_iterator object = objects.begin(); object < objects.end();
         object++) {
  ...
  template <typename PointT>
  Visualizer3D& addLine(const PointT& p1, const PointT& p2, float r = -1.0, float g = -1.0, float b = -1.0) {
    r = (r >= 0) ? r : rngF();
    g = (g >= 0) ? g : rngF();
    b = (b >= 0) ? b : rngF();
