...
class ObjectState {
 public:
  typedef enum Event { ADDED, REMOVED, PRESERVED, MOVED } EventT;
...
class ObjectHistory {
 public:
  ...
  void markRemovedIfNotDetected(int timestamp) {
    if (historyStates.back().getTimestamp() < timestamp && historyStates.back().getEvent() != ObjectState::REMOVED) {
  ...
  void markPreservedIfNotDetectedNorRemoved(int timestamp) {
    if (historyStates.back().getTimestamp() < timestamp && historyStates.back().getEvent() != ObjectState::REMOVED) {
      historyStates.push_back(
          ObjectState::Preserved(timestamp, historyStates.back().getId(), historyStates.back().getPose()));
    }
  }
...
class ObjectIdLabeled {
 public:
  ...
  ObjectIdLabeled(const int id_, const ObjectLabel &label_) : id(id_), label(label_) {}
};
