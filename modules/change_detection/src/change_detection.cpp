...
void ChangeDetector<PointT>::detect(const typename pcl::PointCloud<PointT>::ConstPtr &old_scene,
                                    const CloudPtr new_scene, const Eigen::Affine3f sensor_pose, float diff_tolerance) {
  added->clear();
  removed->clear();
  ...
  *added += *(differenceNew);
  ...
  if (!differenceOld->empty()) {
    ...
    typename OcclusionChecker<PointT>::occlusion_results occlusions_old =
        occlusionChecker.checkOcclusions(differenceOld, new_scene);
...
template <class PointT>
typename ChangeDetector<PointT>::CloudPtr ChangeDetector<PointT>::removalSupport(CloudPtr &object_cloud) const {
  return removalSupport(removed, object_cloud);
}

template <class PointT>
int ChangeDetector<PointT>::overlapingPoints(const CloudPtr train, const CloudPtr query, float tolerance) {
 ...
 */
template <class PointT>
float ChangeDetector<PointT>::computePlanarity(const typename pcl::PointCloud<PointT>::ConstPtr input_cloud) {
...
template <class PointT>
std::vector<typename pcl::PointCloud<PointT>::Ptr> ChangeDetector<PointT>::clusterPointCloud(CloudPtr input_cloud,
                                                                                             double tolerance,
