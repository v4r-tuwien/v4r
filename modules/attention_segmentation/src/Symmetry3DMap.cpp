...
Symmetry3DMap::Symmetry3DMap() : BaseMap() {
  reset();
}
...
void Symmetry3DMap::createLookUpMap(pcl::PointIndices::Ptr indices_cur, unsigned int width_cur, unsigned int height_cur,
                                    cv::Mat &lookupMap) {
...
void Symmetry3DMap::symmetry3DMap(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_cur,
                                  pcl::PointCloud<pcl::Normal>::Ptr normals_cur, pcl::PointIndices::Ptr indices_cur,
  ...
  for (unsigned int idx = 0; idx < indices_cur->indices.size(); ++idx) {
    ...
    for (unsigned int axis_num = 0; axis_num < axis.size(); ++axis_num) {
      ...
      if ((leftPoint.num > 0) && (rightPoint.num > 0)) {
        ...
        float d = plane_normal.normal[0] * (leftPoint.point.x - point0.x) +
                  plane_normal.normal[1] * (leftPoint.point.y - point0.y) +
                  plane_normal.normal[2] * (leftPoint.point.z - point0.z);
        ...
        if ((leftPoint.point.z > 0) && (rightPoint.point.z > 0) && (rightPoint.distance > 0) &&
            (leftPoint.distance > 0) && point_is_ok) {
...
int Symmetry3DMap::combinePyramid(BasePyramid::Ptr pyramid) {
  for (unsigned int i = pyramid->getStartLevel(); i <= (unsigned int)pyramid->getMaxLevel(); ++i) {
    ...
    cv::Mat current_map;
    symmetry3DMap(current_cloud, current_normals, current_indices, current_width, current_height, R, current_map,
                  filter_size);
