...
bool SequentialFitter::is_back_facing(const Eigen::Vector3d &v0, const Eigen::Vector3d &v1, const Eigen::Vector3d &v2,
...
void SequentialFitter::setCorners(pcl::PointIndices::Ptr &corners, bool flip_on_demand) {
  if (m_cloud.get() == 0 || m_cloud->points.size() == 0)
...
void SequentialFitter::setProjectionMatrix(const Eigen::Matrix4d &intrinsic, const Eigen::Matrix4d &extrinsic) {
...
void SequentialFitter::getClosestPointOnNurbs(ON_NurbsSurface nurbs, const Eigen::Vector3d &pt, Eigen::Vector2d &params,
...
unsigned SequentialFitter::PCL2ON(pcl::PointCloud<pcl::PointXYZRGB>::Ptr &pcl_cloud, const std::vector<int> &indices,
