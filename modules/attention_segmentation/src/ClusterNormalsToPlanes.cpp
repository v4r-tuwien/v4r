 ...
 */
ClusterNormalsToPlanes::ClusterNormalsToPlanes(Parameter p) : EPBase() {
  setParameter(p);
  pixel_check = false;
  max_neighbours = 4;
 ...
 */
void ClusterNormalsToPlanes::countNeighbours(std::vector<std::vector<int>> &reassign_idxs, int nb, int nnb,
                                             int nnb_inc) {
  reassign_idxs.clear();
...
//
//         // update normal
//         normal.getNormalVector3fMap() = ((pts.size())*normal.getNormalVector3fMap() +
//         view->normals->points.at(idx).getNormalVector3fMap()) / ((float)pts.size() + 1.0f);
...
void ClusterNormalsToPlanes::printCloudAdaptiveParams(std::string file_name) {
  ...
  for (unsigned int i = 0; i < p_adaptive_cosThrAngleNC.size(); i++) {
    Eigen::Vector3f curPt = cloud->points.at(i).getVector3fMap();
    fprintf(f, "(%d,%d): %f,%f,%f -- %f %f \n", i % width, i / width, curPt[0], curPt[1], curPt[2],
            p_adaptive_cosThrAngleNC[i], p_adaptive_inlDist[i]);
  }
  fclose(f);
}
...
void ClusterNormalsToPlanes::computeLeastSquarePlanes() {
  ...
  for (unsigned int i = 0; i < surfaces.size(); i++) {
    if ((surfaces.at(i)->type == pcl::SACMODEL_PLANE) && (surfaces.at(i)->indices.size() > 4)) {
      ...
      if ((coeffs[0] * currentCoeffs[0] + coeffs[1] * currentCoeffs[1] + coeffs[2] * currentCoeffs[2]) >
          cosThrAngleNC) {
...
void ClusterNormalsToPlanes::print(std::string file_name) {
  ...
  for (unsigned int i = 0; i < surfaces.size(); ++i) {
    ...
    for (unsigned int j = 0; j < surfaces.at(i)->indices.size(); ++j) {
      fprintf(f, "%d %f %f %f\n", surfaces.at(i)->indices.at(j), surfaces.at(i)->normals.at(j)[0],
              surfaces.at(i)->normals.at(j)[1], surfaces.at(i)->normals.at(j)[2]);
    }
    fprintf(f, "\n");
  }
  fclose(f);
