...
void calculatePolygonsCenters(std::vector<std::vector<cv::Point>> &polygons, std::vector<cv::Point> &attentionPoints,
...
void makeGaborKernel2D(cv::Mat &kernel, float &max_sum, float theta, float bandwidth, float lambda, float sigma,
