...
void StructuralRelations::init() {
  if ((!have_cloud) || (!have_surfaces) || (!have_normals) || (!have_neighbours2D) ||
      (!have_neighbours3D))  //|| (!have_relations))
  {
    char* error_message = new char[200];
    sprintf(error_message, "[%s::compute()]: I suggest you first set the point cloud, surfaces, and neighbours.",
            ClassName.c_str());
...
StructuralRelations::StructuralRelations() : EPBase() {
  have_surfaces = false;
...
void StructuralRelations::setNeighbours3D(
    const std::map<borderIdentification, std::vector<neighboringPair>> _ngbr3D_map) {
...
#pragma omp critical
    ...
    if (currentNeigboringBoundary3D.size() > 0) {
      ...
      if (usedRelations & R_3D2) {
        double ratio3Dboundary2Dboundary =
            ((double)currentNeigboringBoundary3D.size()) / ((double)currentNeigboringBoundary2D.size());
