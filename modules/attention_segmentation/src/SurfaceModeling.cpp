...
bool SurfaceModeling::replacePlaneWithBetterNurbs(SurfaceModel::Ptr &surf) {
  ...
  if (((int)surf->indices.size()) < param.planePointsFixation) {
    ...
    surf->savings =
        computeSavingsNormalized(COSTS_PLANE_PARAMS, surf->probs, surf->indices.size(), param.kappa1, param.kappa2);
    fitNurbs(model);
 ...
 */
bool SurfaceModeling::tryMergeSurfaces(SurfaceModel::Ptr surf1, SurfaceModel::Ptr surf2,
                                       SurfaceModel::Ptr &mergedSurf) {
  if ((surf1->indices.size() + surf2->indices.size()) > 3) {
    ...
    *mergedSurf = *surf1;
    ...
    fitNurbs(mergedSurf);
    mergedSurf->savings =
        computeSavingsNormalized(mergedSurf->nurbs.m_cv_count[0] * mergedSurf->nurbs.m_cv_count[1] * COSTS_NURBS_PARAMS,
    ...
    surf1->savings = computeSavingsNormalized(
        (surf1->type == MODEL_NURBS ? surf1->nurbs.m_cv_count[0] * surf1->nurbs.m_cv_count[1] * COSTS_NURBS_PARAMS
                                    : COSTS_PLANE_PARAMS),
        surf1->probs, mergedSurf->indices.size(), param.kappa1, param.kappa2);

    surf2->savings = computeSavingsNormalized(
        (surf2->type == MODEL_NURBS ? surf2->nurbs.m_cv_count[0] * surf2->nurbs.m_cv_count[1] * COSTS_NURBS_PARAMS
                                    : COSTS_PLANE_PARAMS),
 ...
 * @return smart pointer to merged surface, or null pointer
 */
bool SurfaceModeling::tryMergeSurfacesWithPlanes(SurfaceModel::Ptr surf1, SurfaceModel::Ptr surf2,
                                                 SurfaceModel::Ptr &mergedSurf) {
  mergedSurf.reset(new SurfaceModel());
  *mergedSurf = *surf1;
  surf2->addTo(*mergedSurf);
...
#pragma omp parallel for
    for (unsigned int i = 0; i < surfaces.size(); i++) {
      if ((!(surfaces.at(i)->isNew)) || (!(surfaces.at(i)->selected)) || (!(surfaces.at(i)->valid)) ||
          (surfaces.at(i)->type != pcl::SACMODEL_PLANE))  //@ep: TODO back??? (surfaces.at(i)->type == MODEL_NURBS))
        continue;
  ...
  for (unsigned int i = 0; i < mergePairs.size(); i++) {
    ...
    if (tryMergeNurbs) {
      ...
      toBeMerged =
          tryMergeSurfacesWithPlanes(surfaces.at(mergePairs.at(i).id1), surfaces.at(mergePairs.at(i).id2), mergedModel);
    }
...
#pragma omp parallel for shared(mergePairs)
  for (unsigned int i = 0; i < surfaces.size(); i++) {
    ...
    if ((surfaces.at(i)->isNew) && (surfaces.at(i)->selected) && (surfaces.at(i)->valid)) {
      ...
      for (std::set<unsigned>::iterator itr = surfaces.at(i)->neighbors3D.begin();
           itr != surfaces.at(i)->neighbors3D.end(); itr++) {
...
void SurfaceModeling::modifyNeighbours(int oldIdx, int newIdx) {
  ...
  for (std::set<unsigned>::iterator itr = surfaces.at(oldIdx)->neighbors3D.begin();
       itr != surfaces.at(oldIdx)->neighbors3D.end(); itr++) {
  ...
  for (std::set<unsigned>::iterator itr = surfaces.at(oldIdx)->neighbors2D.begin();
       itr != surfaces.at(oldIdx)->neighbors2D.end(); itr++) {
    if ((*itr) != ((unsigned int)newIdx)) {
  ...
  for (unsigned int i = 0; i < surfaces.size(); ++i) {
    ...
    if (it2D_current != ngbr2D_map.end()) {
      ...
      if (it2D_temp != ngbr2D_map.end()) {
        (it2D_temp->second)
            .insert((it2D_temp->second).end(), (it2D_current->second).begin(), (it2D_current->second).end());
      } else {
    ...
    if (it3D_current != ngbr3D_map.end()) {
      ...
      if (it3D_temp != ngbr3D_map.end()) {
        (it3D_temp->second)
            .insert((it3D_temp->second).end(), (it3D_current->second).begin(), (it3D_current->second).end());
      } else {
        std::pair<borderIdentification, std::vector<neighboringPair>> new_border;
        new_border.first = borderId_temp;
...
void SurfaceModeling::createNeighbours() {
  ...
  for (unsigned int i = 0; i < surfaces.size(); i++) {
    ...
    // go over all neighbors
    for (std::set<unsigned>::iterator itr = surfaces.at(i)->neighbors2D.begin();
         itr != surfaces.at(i)->neighbors2D.end(); itr++) {
...
void SurfaceModeling::computePointError(SurfaceModel::Ptr surf) {
  ...
  if (surf->type == pcl::SACMODEL_PLANE) {
    ...
    for (unsigned int i = 0; i < surf->indices.size(); i++) {
      ...
      surf->error.at(i) =
          fabs(a * curPoint[0] + b * curPoint[1] + c * curPoint[2] + d) / std::sqrt(a * a + b * b + c * c);
    }
  } else {
...
void SurfaceModeling::printSurfaces(std::string file_name) {
  ...
  for (unsigned int s = 0; s < surfaces.size(); ++s) {
    ...
    for (unsigned int i = 0; i < surfaces.at(s)->indices.size(); ++i) {
      fprintf(f, "%d %f %f %f\n", surfaces.at(s)->indices.at(i), surfaces.at(s)->normals.at(i)[0],
              surfaces.at(s)->normals.at(i)[1], surfaces.at(s)->normals.at(i)[2]);
    }
    fprintf(f, "\n");
  }
...
void SurfaceModeling::compute() {
  if ((!have_surfaces) || (!have_cloud) || (!have_normals) || (!have_boundary2D) || (!have_boundary3D)) {
    ...
    sprintf(error_message, "[%s::compute()]: I suggest you first set the point cloud, normals, boundary and surfaces.",
            ClassName.c_str());
