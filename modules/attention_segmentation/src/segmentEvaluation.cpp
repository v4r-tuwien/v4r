...
void printSegmentationEvaluation(std::string output_filename, std::string base_name, std::vector<long int> &tp,
  ...
  for (size_t i = 1; i < objNumber.size(); ++i) {
    ...
    fprintf(f, "%12s %7d %7d %8ld %8ld %8ld %5.4f %5.4f %5.4f\n", path.filename().c_str(), (int)i, objNumber.at(i),
            tp.at(i), fp.at(i), fn.at(i), precision, recall, fscore);
  }

  fclose(f);
}

void evaluate(pcl::PointCloud<pcl::PointXYZRGBL>::Ptr pcl_cloud_l, cv::Mat &mask, std::string base_name,
...
void evaluate(pcl::PointCloud<pcl::PointXYZRGBL>::Ptr pcl_cloud_l, cv::Mat &mask, cv::Point attention_point,
...
void evaluate(const cv::Mat &ground_truth_image, cv::Mat &mask, cv::Point attention_point, std::string base_name,
              std::string output_filename) {
  std::vector<long int> tp;
  std::vector<long int> fp;
  std::vector<long int> fn;
