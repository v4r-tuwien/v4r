...
void SVMFileCreator::process() {
  ...
  if (!testset) {
    ...
    //
    //           std::fprintf(PPfileAnaRel,"%u: [%u][%u] (%u), ", counter, relations.at(i).id_0, relations.at(i).id_1,
    //           relations.at(i).groundTruth);
    //
    // 	  if(featureNumber == -1)
    // 	  {
    // 	    for(int j=0; j<relations.at(i).rel_value.size(); j++)
    ...
    //
    //           std::fprintf(PP2fileAnaRel,"%u: [%u][%u] (%u), ", counter, relations.at(i).id_0, relations.at(i).id_1,
    //           relations.at(i).groundTruth);
    //           for(int j=0; j<relations.at(i).rel_value.size(); j++)
  ...
  //     for(int i=0; i<relations.size(); i++)
  //     {
  //       if(relations.at(i).groundTruth == 0  || relations.at(i).groundTruth == 1)   // write only relations with
  //       known groundTruth
  //       {
  //         if(relations.at(i).type == 1)
  //         {
  ...
  //       static int counter = 0;
  //
  //       FILE *PPfileAna = std::fopen("./PP-Testset.txt.ana", "a");                           // All relations (with
  //       unknown ones)
  ...
  //       FILE *PPfileAnaRel = std::fopen("./PP-Testset.txt.rel.ana", "a");                    // All relations (with
  //       unknown ones)
  //       FILE *PP2fileAnaRel = std::fopen("./PP2-Testset.txt.rel.ana", "a");                  // All relations (with
  //       unknown ones)
  //
  //       for(int i=0; i<relations.size(); i++)
  //       {
