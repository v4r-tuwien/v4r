  ...
  for (int i = 0; i < salmap.rows; i++) {
    for (int j = 0; j < salmap.cols; j++) {
      wta.sm.I.at<float>(i, j) = salmap.at<float>(i, j) * salParams.smOutputRange +
                                 salParams.noiseAmpl * (((double)rand()) / RAND_MAX) + salParams.noiseConst;
    }
  }
...
void UpdateWinner2(cv::Mat& salmap, cv::Point& winner) {
  ...
  while (i < points_size) {
    ...
    if (((x + 1) < salmap.cols) && (salmap.at<double>(y, x + 1) > salmap.at<double>(y, x)) &&
    ...
    if (((y + 1) < salmap.rows) && (salmap.at<double>(y + 1, x) > salmap.at<double>(y, x)) &&
        (!processed.at<uchar>(y + 1, x))) {
      total_x += x;
      total_y += y + 1;
      total += 1;
      points.at(points_size) = cv::Point(x, y + 1);
...
int CalculateWTA(cv::Mat& img, cv::Mat& _salmap, std::vector<cv::Point>& attented_points, int AttentionPointsNumber,
                 Params& params) {
  cv::Mat temp;
  _salmap.copyTo(temp);
  ...
  cv::Mat salmap;
  cv::resize(temp, salmap,
             cv::Size(_salmap.cols / (pow(2, params.mapLevel - 1)), _salmap.rows / (pow(2, params.mapLevel - 1))));
