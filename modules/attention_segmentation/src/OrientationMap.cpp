...
OrientationSaliencyMap::OrientationSaliencyMap() : BaseMap() {
  reset();
}
...
void OrientationSaliencyMap::orientationMap(cv::Mat &image_cur, int image_width, int image_height, float angle,
                                            float max_sum, float bandwidth, cv::Mat &map_cur) {
    ...
    for (int c = 0; c < image_width; ++c) {
      ...
      for (int j = r - gaborFilerSize; j <= r + gaborFilerSize; ++j)  // rows
      {
        ...
        for (int i = c - gaborFilerSize; i <= c + gaborFilerSize; ++i)  // cols
        {
          ...
          value +=
              image_cur.at<float>(yy, xx) * gaborKernel.at<float>(j - (r - gaborFilerSize), (i - (c - gaborFilerSize)));
        }
      }
