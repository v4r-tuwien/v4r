...
void pointCloud_2_depth(cv::Mat &depth, pcl::PointCloud<pcl::PointXYZRGBL>::ConstPtr cloud, unsigned int width,
                        unsigned int height, pcl::PointIndices::Ptr indices, float th) {
...
void pointCloud_2_depth(cv::Mat &depth, pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr cloud, unsigned int width,
                        unsigned int height, pcl::PointIndices::Ptr indices, float th) {
  if ((indices->indices.size()) == 0) {
    indices->indices.resize(cloud->size());
    for (unsigned int i = 0; i < cloud->size(); ++i) {
...
void pointCloud_2_rgb(cv::Mat &RGB, pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr cloud_xyzrgb, unsigned int width,
                      unsigned int height, pcl::PointIndices::Ptr indices) {
  if ((indices->indices.size()) == 0) {
...
void pointCloud_2_channels(cv::Mat &xchannel, cv::Mat &ychannel, cv::Mat &zchannel,
                           pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr cloud, unsigned int width, unsigned int height,
                           pcl::PointIndices::Ptr indices, float th) {
  if ((indices->indices.size()) == 0) {
    indices->indices.resize(cloud->size());
    for (unsigned int i = 0; i < cloud->size(); ++i) {
      indices->indices.at(i) = i;
...
void normals_2_channels(cv::Mat &xnormals, cv::Mat &ynormals, cv::Mat &znormals,
                        pcl::PointCloud<pcl::Normal>::ConstPtr normals, unsigned int width, unsigned int height,
...
void pointCloud_2_disparity(cv::Mat &disparity, pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr cloud, int width,
                            int height, pcl::PointIndices::Ptr indices, float f, float b, float th) {
  if ((indices->indices.size()) == 0) {
    indices->indices.resize(cloud->size());
    for (unsigned int i = 0; i < cloud->size(); ++i) {
      indices->indices.at(i) = i;
...
void depthRGB_2_cloudXYZRGB(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_xyzrgb, const cv::Mat &depth,
                            const cv::Mat &RGB, const std::vector<float> &cameraParametrs, unsigned int width,
                            unsigned int height, float th) {
  // check types
...
void pointCloudXYZimageRGBimageL_2_cloudXYZRGBL(pcl::PointCloud<pcl::PointXYZRGBL>::Ptr cloud_xyzrgbl,
                                                pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_xyz, cv::Mat &RGB, cv::Mat &L,
                                                unsigned int width, unsigned int height) {
  assert(cloud_xyz->height == (unsigned int)RGB.rows);
  assert(cloud_xyz->width == (unsigned int)RGB.cols);
...
void pointCloudXYZRGBL_2_cloudXYZimageRGBlableL(pcl::PointCloud<pcl::PointXYZRGBL>::Ptr cloud_xyzrgbl,
                                                pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_xyz, cv::Mat &RGB, cv::Mat &L,
                                                unsigned int width, unsigned int height) {
...
void pointCloudXYZRGBimageL_2_cloudXYZRGBL(pcl::PointCloud<pcl::PointXYZRGBL>::Ptr cloud_xyzrgbl,
...
void pointCloudXYZRGBL_2_cloudXYZRGBlableL(pcl::PointCloud<pcl::PointXYZRGBL>::Ptr cloud_xyzrgbl,
                                           pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_xyzrgb, cv::Mat &L,
                                           unsigned int width, unsigned int height) {
  assert(height == cloud_xyzrgbl->height);
  assert(width == cloud_xyzrgbl->width);
