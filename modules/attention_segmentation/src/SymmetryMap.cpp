...
SymmetryMap::SymmetryMap() : BaseMap() {
  reset();
}
...
void SymmetryMap::symmetryMap(cv::Mat &image_cur, int image_width, int image_height, int R1, int R2, int S,
                              cv::Mat &map_cur) {
  (void)S;
  ...
  for (int y = 0; y < image_height; ++y)  // START of Iteration over all Pixel ***************************
  {
    for (int x = 0; x < image_width; ++x) {
      ...
      for (int j = dy; j < (R2 + 1);
           j++)  // Start Iteration over the Mask (Boundary Box) --------------------------------------------
      {
        for (int i = dx; i < (R2 * 2 + 1 - dx); i++) {
          ...
          if (!((j > (R2 - R1)) && (j < (R2 + R1)) && (i > (R2 - R1)) && (i < (R2 + R1)))) {
            ...
            symV = (1 - cosAr.at<float>((int)(cosArSize * (g0 + g1) / (4 * M_PI)) + cosArSize, 1)) *
                   (1 - cosAr.at<float>((int)(cosArSize * (g0 - g1) / (4 * M_PI)) + cosArSize, 1)) * gwf *
                   distanceWeight.at(d);
