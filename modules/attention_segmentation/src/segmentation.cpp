...
void Segmenter::createTrainFile() {
  if ((!have_cloud) || (!have_cloud_l)) {
    ...
    sprintf(error_message, "[%s::segment()]: I suggest you first set the point cloud.",
            ClassName.c_str());  // and normals
...
void Segmenter::attentionSegment() {
  ...
  v4r::TimeEstimationClass timeEstimationClass_All(
      CLOCK_THREAD_CPUTIME_ID);  // CLOCK_THREAD_CPUTIME_ID);//CLOCK_PROCESS_CPUTIME_ID
...
int Segmenter::attentionSegment(cv::Mat& object_mask, int originalIndex, int salMapNumber) {
  ...
  while (true) {
    ...
    computeRelations();
    timeEstimationClass_CustomLocal.countingEnd();
    timeEstimates.times_relationsComputation.at(salMapNumber) +=
        timeEstimationClass_CustomLocal.getWorkTimeInNanoseconds();

    timeEstimationClass_CustomLocal.countingStart();
    graphBasedSegmentation();
    timeEstimationClass_CustomLocal.countingEnd();
    timeEstimates.times_graphBasedSegmentation.at(salMapNumber) +=
        timeEstimationClass_CustomLocal.getWorkTimeInNanoseconds();
    ...
    for (size_t i = 0; i < selected_surfaces.size(); ++i) {
      ...
      for (std::set<unsigned>::iterator itr = surfaces.at(idx)->neighbors3D.begin();
           itr != surfaces.at(idx)->neighbors3D.end(); itr++) {
        if ((surfaces.at(*itr)->valid) && (!(surfaces.at(*itr)->selected))) {
          //           printf("Adding neighbour %u is selected \n",*itr);
