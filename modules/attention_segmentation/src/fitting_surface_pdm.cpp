...
void FittingSurface::setInvMapParams(unsigned in_max_steps, double in_accuracy) {
  this->in_max_steps = in_max_steps;
  this->in_accuracy = in_accuracy;
}

std::vector<double> FittingSurface::getElementVector(const ON_NurbsSurface &nurbs, int dim)  // !
{
...
ON_NurbsSurface FittingSurface::initNurbs4Corners(int order, ON_3dPoint ll, ON_3dPoint lr, ON_3dPoint ur,
                                                  ON_3dPoint ul) {
...
ON_NurbsSurface FittingSurface::initNurbsPCA(int order, NurbsDataSurface *m_data, Eigen::Vector3d z) {
  Eigen::Vector3d mean;
...
ON_NurbsSurface FittingSurface::initNurbsPCABoundingBox(int order, NurbsDataSurface *m_data, Eigen::Vector3d z) {
  Eigen::Vector3d mean;
  Eigen::Matrix3d eigenvectors;
  Eigen::Vector3d eigenvalues;
...
void FittingSurface::addPointConstraint(const Eigen::Vector2d &params, const Eigen::Vector3d &point, double weight,
                                        unsigned &row) {
...
void FittingSurface::addCageBoundaryRegularisation(double weight, int side, unsigned &row) {
  int i = 0;
  int j = 0;
...
void FittingSurface::addInteriorRegularisation(int order, int resU, int resV, double weight, unsigned &row) {
  double *N0 = new double[m_nurbs.Order(0) * m_nurbs.Order(0)];
  double *N1 = new double[m_nurbs.Order(1) * m_nurbs.Order(1)];
...
void FittingSurface::addBoundaryRegularisation(int order, int resU, int resV, double weight, unsigned &row) {
  double *N0 = new double[m_nurbs.Order(0) * m_nurbs.Order(0)];
  double *N1 = new double[m_nurbs.Order(1) * m_nurbs.Order(1)];
...
Vector2d FittingSurface::inverseMapping(const ON_NurbsSurface &nurbs, const Vector3d &pt, const Vector2d &hint,
...
Vector2d FittingSurface::inverseMapping(const ON_NurbsSurface &nurbs, const Vector3d &pt, const Vector2d &hint,
                                        Vector3d &p, int maxSteps, double accuracy, bool quiet) {
  double pointAndTangents[9];
...
Vector2d FittingSurface::findClosestElementMidPoint(const ON_NurbsSurface &nurbs, const Vector3d &pt) {
  Vector2d hint;
  Vector3d r;
  std::vector<double> elementsU = getElementVector(nurbs, 0);
  std::vector<double> elementsV = getElementVector(nurbs, 1);
...
Vector2d FittingSurface::inverseMappingBoundary(const ON_NurbsSurface &nurbs, const Vector3d &pt, double &error,
                                                Vector3d &p, Vector3d &tu, Vector3d &tv, int maxSteps, double accuracy,
                                                bool quiet) {
  Vector2d result;
  ...
  for (unsigned i = 0; i < ini_points.size(); i++) {
    Vector2d params = inverseMappingBoundary(nurbs, pt, ini_points[i].side, ini_points[i].hint, err_tmp, p_tmp, tu_tmp,
...
Vector2d FittingSurface::inverseMappingBoundary(const ON_NurbsSurface &nurbs, const Vector3d &pt, int side, double hint,
  ...
  if (!quiet)
    printf(
        "[FittingSurface::inverseMappingBoundary] Warning: Method did not converge! (residual: %f, delta: %f, params: "
        "%f %f)\n",
