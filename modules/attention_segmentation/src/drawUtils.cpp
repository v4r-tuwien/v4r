...
void drawSegmentationResults(cv::Mat &image, cv::Point p1, cv::Mat &masks, bool drawAttentionPoints,
  ...
  if (drawAttentionPoints) {
    ...
    char text[4];
    sprintf(text, "%d", num);
    cv::putText(image, text, cv::Point(p1.x, p1.y - 5), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 0, 0),
                2);  // colors.at(j),2);
...
void drawSegmentationResults(cv::Mat &image, std::vector<cv::Point> &attentionPoints, std::vector<cv::Mat> &binMasks,
                             std::vector<std::vector<cv::Point>> &contours, bool drawAttentionPoints,
                             bool drawSegmentationResults) {
  cv::Point p;
  unsigned int N = (attentionPoints.size() > binMasks.size() ? attentionPoints.size() : attentionPoints.size());
  for (unsigned int j = 0; j < N; ++j) {
    // draw contour
    int r = rand() % 256;
    int g = rand() % 256;
