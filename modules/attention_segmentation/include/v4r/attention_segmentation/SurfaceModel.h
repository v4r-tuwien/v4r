...
class SurfaceModel {
 ...
 public:
  ...
  SurfaceModel()
  : idx(-1), label(-1), label_ass(-1), selected(true), valid(true), initialized(false), isNew(true),
    segmented_number(-1) {}
  SurfaceModel(int _idx)
  : idx(_idx), label(-1), label_ass(-1), selected(true), valid(true), initialized(false), isNew(true),
    segmented_number(-1) {}
  ...
  void Print() {
    ...
    for (NeighborIter i = neighbors3D.begin(); i != neighbors3D.end(); i++)
      printf("%d ", (int)*i);
    printf("\n  ");
    printf("  nurbs: order: %dx%d, control-points: %dx%d\n  ", nurbs.Order(0), nurbs.Order(1), nurbs.CVCount(0),
           nurbs.CVCount(1));
...
inline std::vector<v4r::SurfaceModel::Ptr> deepCopy(const std::vector<v4r::SurfaceModel::Ptr> &_surfaces) {
...
inline std::vector<v4r::SurfaceModel::Ptr> deepCopyType(const std::vector<v4r::SurfaceModel::Ptr> &_surfaces,
                                                        int type) {
  std::vector<v4r::SurfaceModel::Ptr> surfaces;
  for (unsigned i = 0; i < _surfaces.size(); i++) {
