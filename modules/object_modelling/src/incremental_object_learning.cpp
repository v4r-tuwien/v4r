...
boost::dynamic_bitset<> IOL::extractEuclideanClustersSmooth(const pcl::PointCloud<PointT>::ConstPtr &cloud,
  ...
  while (!stop) {
    ...
    for (size_t i = 0; i < cloud->points.size(); i++) {
      ...
      if (octree.radiusSearch(cloud->points[i], param_.radius_, nn_indices, nn_distances)) {
        ...
        {
          if (!in_cluster[nn_indices[j]] && !bg_mask[nn_indices[j]])  // if nearest neighbor is not already an object
                                                                      // and is not a point to be neglected (background)
          {
...
void IOL::updatePointNormalsFromSuperVoxels(const pcl::PointCloud<PointT>::Ptr &cloud,
  ...
  //    const pcl::PointCloud<pcl::PointNormal>::Ptr sv_normal_cloud = super.makeSupervoxelNormalCloud
  //    (supervoxel_clusters);
  ...
  for (size_t i = 0; i < cloud->points.size(); i++) {
    ...
    if (obj_mask[i] && label == 0)  // label 0 means the point could not be associated to any supervoxel (see
                                    // supervoxelclustering doc) - we will pass this point therefore by definition
    {
      obj_mask_out[i] = true;
      continue;
    }
...
void IOL::nnSearch(const pcl::PointCloud<PointT> &object_points, const pcl::PointCloud<PointT>::ConstPtr &search_cloud,
...
void IOL::nnSearch(const pcl::PointCloud<PointT> &object_points, pcl::octree::OctreePointCloudSearch<PointT> &octree,
                   boost::dynamic_bitset<> &obj_mask) {
  ...
  for (size_t i = 0; i < object_points.points.size(); i++) {
    if (!pcl::isFinite(object_points.points[i])) {
      PCL_WARN("Warning: Point is NaN.\n");  // not sure if this causes somewhere else a problem. This condition should
                                             // not be fulfilled.
      continue;
    }
    if (octree.radiusSearch(object_points.points[i], param_.radius_, pointIdxRadiusSearch, pointRadiusSquaredDistance) >
        0) {
...
boost::dynamic_bitset<> IOL::erodeIndices(const boost::dynamic_bitset<> &obj_mask,
  ...
  for (int r = 0; r < mask_dst.rows; r++) {
    for (int c = 0; c < mask_dst.cols; c++) {
      ...
      if (mask_dst.at<unsigned char>(r, c) > 0 && pcl::isFinite(cloud.points[idx]) &&
...
bool IOL::save_model(const std::string &models_dir, const std::string &model_name, bool save_individual_views) {
  ...
  if (kept_keyframes > 0) {
    ...
    pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr filtered_with_normals_oriented(
...
void IOL::extractPlanePoints(const pcl::PointCloud<PointT>::ConstPtr &cloud,
...
bool IOL::merging_planes_reasonable(const modelView::SuperPlane &sp1, const modelView::SuperPlane &sp2) const {
...
void IOL::computePlaneProperties(const std::vector<ClusterNormalsToPlanes::Plane::Ptr> &planes,
  ...
  for (size_t cluster_id = 0; cluster_id < planes.size(); cluster_id++) {
    ...
    //        if ( num_plane_pts == 0 ||
    //             ( (double)num_obj_pts/num_plane_pts < param_.ratio_cluster_obj_supported_ &&
    //             (double)num_occluded_pts/num_plane_pts < param_.ratio_cluster_occluded_) )
    //        {
...
bool IOL::learn_object(const pcl::PointCloud<PointT> &cloud, const Eigen::Matrix4f &camera_pose,
                       const std::vector<size_t> &initial_indices) {
  size_t id = grph_.size();
  std::cout << "Computing indices for cloud " << id << std::endl << "===================================" << std::endl;
  grph_.resize(id + 1);
  ...
  if (initial_indices.size())  // for first frame use given initial indices and erode them
  {
    ...
    for (size_t idx = 0; idx < initial_indices.size(); idx++) {
      if (pcl::isFinite(view.cloud_->points[initial_indices[idx]]) &&
          view.cloud_->points[initial_indices[idx]].z < param_.chop_z_) {
    ...
    *ObjectIndicesPtr = convertVecSizet2VecInt(initial_indices_wo_nan);
    ...
    const boost::dynamic_bitset<> obj_mask_initial =
        createMaskFromIndices(initial_indices_wo_nan, view.cloud_->points.size());
    const boost::dynamic_bitset<> outlier_mask =
        createMaskFromIndices(*FilteredObjectIndicesPtr, view.cloud_->points.size());
    const boost::dynamic_bitset<> obj_mask_wo_outlier =
        binary_operation(obj_mask_initial, outlier_mask, BINARY_OPERATOR::AND_N);
    ...
    computePlaneProperties(planes, view.obj_mask_step_[0], boost::dynamic_bitset<>(view.cloud_->points.size(), 0),
    ...
    computePlaneProperties(planes, obj_mask_nn_search, is_occluded, view.cloud_, view.planes_);
  }
    ...
    for (size_t view_id = 0; view_id < grph_.size(); view_id++) {
      if (view.id_ != grph_[view_id].id_) {
        for (size_t p2_id = 0; p2_id < grph_[view_id].planes_.size(); p2_id++) {
          ...
          //                    std::cout << merging_planes_reasonable(view.planes_[p_id],
          //                    grph_[view_id].planes_[p2_id]) << std::endl;

          // if the planes can be merged (based on normals and distance), then filter new plane if old one has been
          // filtered
          if (grph_[view_id].planes_[p2_id].is_filtered &&
              merging_planes_reasonable(view.planes_[p_id], grph_[view_id].planes_[p2_id]) &&
              !plane_has_object(view.planes_[p_id]))
  ...
  boost::dynamic_bitset<> obj_mask_grown_by_smooth_surface = extractEuclideanClustersSmooth(
      view.cloud_, *view.normal_, obj_mask_enforced_by_supervoxel_consistency, pixel_is_neglected);
  ...
  for (size_t step_id = 0; step_id < view.obj_mask_step_.size(); step_id++) {
    std::cout << "step " << step_id << ": " << createIndicesFromMask<size_t>(view.obj_mask_step_[step_id]).size()
              << " points." << std::endl;
  }

  if (view.is_pre_labelled_ &&
      createIndicesFromMask<size_t>(view.obj_mask_step_.back()).size() < param_.min_points_for_transferring_) {
    view.obj_mask_step_.back() = view.obj_mask_step_[0];
    std::cout << "After postprocessing the initial frame not enough points are left. Therefore taking the original "
                 "provided indices."
              << std::endl;
  }
  //    visualize();
...
void IOL::printParams(std::ostream &ostr) const {
  ostr << "Started incremental object learning with parameters: " << std::endl
       ...
       << "transferring object indices from latest frame only: " << param_.transfer_indices_from_latest_frame_only_
       << std::endl
