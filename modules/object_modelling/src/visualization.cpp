...
void IOL::visualize_clusters() {
  ...
  std::vector<int> viewports =
      v4r::pcl_visualizer::visualization_framework(*vis_seg_, grph_.size(), 3, subwindow_title);
  ...
  for (size_t view_id = 0; view_id < grph_.size(); view_id++) {
    ...
    for (size_t cluster_id = 0; cluster_id < grph_[view_id].planes_.size(); cluster_id++) {
      ...
      const double num_vis = static_cast<double>(grph_[view_id].planes_[cluster_id].visible_indices.size()) /
                             grph_[view_id].planes_[cluster_id].within_chop_z_indices.size();
      const double num_obj = static_cast<double>(grph_[view_id].planes_[cluster_id].object_indices.size()) /
                             grph_[view_id].planes_[cluster_id].within_chop_z_indices.size();
      ...
      if (grph_[view_id].planes_[cluster_id].is_filtered) {
        ...
        vis_seg_->addText(text.str(), 40, 10 * cluster_id, 10, r / 255, g / 255, b / 255, name.str(),
                          viewports[view_id * 3 + 1]);
      } else {
        pcl::visualization::PointCloudColorHandlerCustom<PointT> colored_src(segmented, r, g, b);
...
void IOL::visualize() {
  ...
  for (size_t view_id = 0; view_id < grph_.size(); view_id++) {
    ...
    vis_->addPointCloud(cloud_trans, rgb_handler, cloud_name.str(),
    ...
    if (!grph_[view_id].is_pre_labelled_) {
      ...
      vis_->addPointCloud(cloud_trans_tmp, green_source, cloud_name.str(),
                          vis_viewpoint_[view_id * num_subwindows + subwindow_id++]);
    } else {
      ...
      vis_->addPointCloud(obj_trans_tmp, red_source, cloud_name.str(),
                          vis_viewpoint_[view_id * num_subwindows + subwindow_id++]);
    }
    ...
    for (size_t step_id = 0; step_id < grph_[view_id].obj_mask_step_.size(); step_id++) {
      ...
      vis_->addPointCloud(segmented_trans, rgb_handler_tmp, cloud_name.str(),
                          vis_viewpoint_[view_id * num_subwindows + subwindow_id++]);
    }
  }
  vis_->spin();
  ...
  for (size_t view_id = 0; view_id < grph_.size(); view_id++) {
    ...
    *cloud_with_search_pts += *search_pts;
    ...
    for (size_t step_id = 0; step_id < grph_[view_id].obj_mask_step_.size(); step_id++) {
      ...
      cv::imwrite(filename_step.str(),
                  cropImage(pcl_opencv_converter.getRGBImage(), pcl_opencv_converter.getROI(), 0, false));
    }
  }
}
}  // namespace object_modelling
