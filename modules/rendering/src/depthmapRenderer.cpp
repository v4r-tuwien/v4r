...
int DepthmapRenderer::search_midpoint(int &index_start, int &index_end, size_t &n_vertices, int &edge_walk,
                                      std::vector<int> &midpoint, std::vector<int> &start, std::vector<int> &end,
...
void DepthmapRenderer::subdivide(size_t &n_vertices, size_t &n_edges, size_t &n_faces, std::vector<float> &vertices,
...
DepthmapRenderer::DepthmapRenderer(int resx, int resy) {
  ...
  if (err != GLEW_OK) {
    std::stringstream s;
    s << "glewInit failed, aborting. " << err;
  ...
  const char *geometry =
      "#version 450\n\
            ...
            void main(){\
                ...
                color=colorIn[0];\n\
                if(hasNormal){\n\
                    norm=mat3(transformation)*normalIn[0];\n\
                }\n\
                EmitVertex();\n\
                ...
                color=colorIn[1];\n\
                if(hasNormal){\n\
                    norm=mat3(transformation)*normalIn[1];\n\
                }\n\
                EmitVertex();\n\
                ...
                texPos=texPosIn[2];\n\
                if(hasNormal){\n\
                    norm=mat3(transformation)*normalIn[2];\n\
                }\n\
                EmitVertex();\n\
  ...
  const char *fragment =
      "#version 450 \n\
            ...
            \
               ...
               colorOutput=color;\n\
               normOutput = vec4(norm/length(norm),0.0);\n\
  ...
  glBindFragDataLocation(shaderProgram, 3, "normalOutput");
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, resx, resy, 0, GL_RGBA, GL_FLOAT, 0);

  // z buffer
  glGenRenderbuffers(1, &zBuffer);
  glBindRenderbuffer(GL_RENDERBUFFER, zBuffer);
  glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT32F, resx,
                        resy);  // GL_DEPTH_COMPONENT_32F //without the 32F?
  ...
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT3, GL_TEXTURE_2D, normalTex, 0);
...
std::vector<Eigen::Vector3f> DepthmapRenderer::createSphere(float r, size_t subdivisions) {
  ...
  float icosahedron_vertices[] = {tau, one, 0.0, -tau, one,  0.0,  -tau, -one, 0.0,  tau,  -one, 0.0,
                                  one, 0.0, tau, one,  0.0,  -tau, -one, 0.0,  -tau, -one, 0.0,  tau,
                                  0.0, tau, one, 0.0,  -tau, one,  0.0,  -tau, -one, 0.0,  tau,  -one};
  int icosahedron_faces[] = {4, 8, 7, 4, 7, 9, 5,  6,  11, 5, 10, 6,  0, 4,  3, 0,  3, 5, 2,  7,
...
void DepthmapRenderer::setModel(DepthmapRendererModel *_model) {
  ...
  glVertexAttribPointer(posAttribute, 4, GL_FLOAT, GL_FALSE,
...
Eigen::Matrix4f DepthmapRenderer::getPoseLookingToCenterFrom(const Eigen::Vector3f &position) {
  glm::vec3 up(0, 0, -1);
  ...
  // somehow the camera is not looking in the correct direction
  // rotation around the y axis by 180° does help
  // camera coordinate system of opengl goes negative z
  // for the pinhole model the camera points in positive z direction
  // this also explains why the up vector points in negative z direction....
  // the rotation turns the camera the other direction but also upside down.
  glm::mat4 rot = glm::mat4();  // identit
...
Eigen::Matrix4f DepthmapRenderer::lookAt(const Eigen::Vector3f &from, const Eigen::Vector3f &to,
...
void DepthmapRenderer::setCamPose(const Eigen::Matrix4f &_pose) {
  this->pose = _pose;
}
...
cv::Mat DepthmapRenderer::renderDepthmap(float &visible, cv::Mat &color, cv::Mat &normal) const {
  ...
  Matrix4f proj1;  // one to do what has to be put anyway
  proj1 << fx, 0, cx, 0, 0, fy, cy, 0, 0, 0, 0, -1, 0, 0, 1, 0;
  ...
  proj2 << 2.0f / (w), 0, 0, -1.0f + 1.0f / w, 0, 2.0f / (h), 0, -1.0f + 1.0f / h, 0, 0, b, a, 0, 0, 0, 1;
  proj = proj2 * proj1;
  ...
  // read buffers:
  glBindBuffer(GL_ATOMIC_COUNTER_BUFFER, atomicCounterBuffer);
  ptr = (GLuint *)glMapBufferRange(GL_ATOMIC_COUNTER_BUFFER, 0, sizeof(GLuint) * 2, GL_MAP_READ_BIT);
