...
class V4R_EXPORTS IMKOptimizeModel {
 public:
  class Parameter {
   public:
    ...
    Parameter()
    : eq_pose_dist(0.01), nnr(0.8), pnp_param(RansacSolvePnPdepth::Parameter(1.5, 0.01, 5000, INT_MIN, 4, 0.015)) {}
  };
  class Link {
   public:
    int idx_part;
 ...
 private:
  ...
  void addPoints3d(const std::vector<cv::KeyPoint> &keys, const pcl::PointCloud<pcl::PointXYZRGB> &cloud,
                   const cv::Mat_<unsigned char> &mask, View &view);
  ...
  bool estimatePose(const View &view0, const View &view1, const std::vector<std::vector<cv::DMatch>> &matches,
                    Eigen::Matrix4f &pose, std::vector<int> &_inliers);
  bool validatePoseAndAddLinks(View &view0, View &view1, const std::vector<std::vector<cv::DMatch>> &_matches01,
                               const std::vector<std::vector<cv::DMatch>> &_matches10, const Eigen::Matrix4f &pose01,
                               const Eigen::Matrix4f &pose10, const std::vector<int> &_inliers01,
                               const std::vector<int> &_inliers10);
  void matchViews();

 public:
  cv::Mat dbg;

  IMKOptimizeModel(const Parameter &p, const v4r::FeatureDetector::Ptr &_detector,
