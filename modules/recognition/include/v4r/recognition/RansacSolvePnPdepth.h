...
class V4R_EXPORTS RansacSolvePnPdepth {
 public:
  class Parameter {
   public:
    ...
    : inl_dist_px(_inl_dist_px), eta_ransac(_eta_ransac), max_rand_trials(_max_rand_trials), pnp_method(_pnp_method),
      nb_ransac_points(_nb_ransac_points), inl_dist_z(_inl_dist_z), use_robust_loss(true), loss_scale(1.5),
      depth_error_scale(50) {}
  };
 ...
 private:
  ...
  unsigned countInliers(const std::vector<Eigen::Vector3f> &points, const std::vector<cv::Point2f> &im_points,
                        const std::vector<float> &_inv_depth, const Eigen::Matrix4f &pose);
  void getInliers(const std::vector<Eigen::Vector3f> &points, const std::vector<cv::Point2f> &im_points,
                  const std::vector<float> &_inv_depth, const Eigen::Matrix4f &pose, std::vector<int> &inliers);
  void convertToLM(const std::vector<Eigen::Vector3f> &points, Eigen::Matrix4f &pose);
  void convertFromLM(Eigen::Matrix4f &pose);
  void optimizePoseLM(std::vector<Eigen::Vector3d> &_points3d, const std::vector<cv::Point2f> &_im_points,
 ...
 public:
  ...
  int ransac(const std::vector<Eigen::Vector3f> &points, const std::vector<cv::Point2f> &im_points,
             Eigen::Matrix4f &pose, std::vector<int> &inliers, const std::vector<float> &_depth = std::vector<float>());
  int ransac(const std::vector<Eigen::Vector3f> &_points0, const std::vector<cv::Point2f> &_im_points1,
             const std::vector<Eigen::Vector3f> &_points3d1, Eigen::Matrix4f &pose, std::vector<int> &inliers);
  void setCameraParameter(const cv::Mat &_intrinsic, const cv::Mat &_dist_coeffs);
  void setParameter(const Parameter &_p);

  typedef std::shared_ptr<::v4r::RansacSolvePnPdepth> Ptr;
  typedef std::shared_ptr<::v4r::RansacSolvePnPdepth const> ConstPtr;
};
 ...
 * cvToEigen
 */
inline void RansacSolvePnPdepth::cvToEigen(const cv::Mat_<double> &R, const cv::Mat_<double> &t,
                                           Eigen::Matrix4f &pose) {
  pose.setIdentity();
