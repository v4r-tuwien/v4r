...
typename pcl::PointCloud<PointT>::ConstPtr Model<PointT>::getAssembled(int resolution_mm) const {
  if (resolution_mm <= 0)
    return assembled_;
...
template <typename PointT>
pcl::PointCloud<pcl::Normal>::ConstPtr Model<PointT>::getNormalsAssembled(int resolution_mm) const {
  if (resolution_mm <= 0)
...
template <typename PointT>
pcl::PointCloud<pcl::PointXYZL>::Ptr Model<PointT>::getAssembledSmoothFaces(int resolution_mm) {
  if (resolution_mm <= 0)
...
template <typename PointT>
void Model<PointT>::initialize(const bf::path &model_filename) {
  ...
  if (!io::existsFile(model_filename) || pcl::io::loadPCDFile(model_filename.string(), *all_assembled) == -1) {
    ...
    pcl::io::loadPCDFile(model_filename.string(), *all_assembled);
