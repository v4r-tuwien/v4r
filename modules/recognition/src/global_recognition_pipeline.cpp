...
template <typename PointT>
void GlobalRecognitionPipeline<PointT>::visualize() {
  ...
  if (!vis_param_->no_text_)
    vis_->addText("input cloud", 10, 10, vis_param_->fontsize_, vis_param_->text_color_[0], vis_param_->text_color_[1],
                  vis_param_->text_color_[2], "input", vp1_);
  ...
  if (!vis_param_->no_text_)
    vis_->addText("segments", 10, 10, vis_param_->fontsize_, vis_param_->text_color_[0], vis_param_->text_color_[1],
                  vis_param_->text_color_[2], "segments", vp2_);
  ...
  if (table_plane_set_) {
    ...
    if (!vis_param_->no_text_)
      vis_->addText("table plane", 10, 10, vis_param_->fontsize_, vis_param_->text_color_[0],
                    vis_param_->text_color_[1], vis_param_->text_color_[2], "table plane", vp3_);
  }
  ...
  for (size_t i = 0; i < obj_hypotheses_wo_elongation_check_.size(); i++) {
    ...
    for (size_t k = 0; k < ohs.ohs_.size(); k++) {
      bool found_model;
      typename Model<PointT>::ConstPtr m =
          m_db_->getModelById(ohs.ohs_[k]->class_id_, ohs.ohs_[k]->model_id_, found_model);
  ...
  if (!vis_param_->no_text_)
    vis_->addText("all hypotheses", 10, 10, vis_param_->fontsize_, vis_param_->text_color_[0],
                  vis_param_->text_color_[1], vis_param_->text_color_[2], "all_hypotheses", vp5_);
  ...
  if (!vis_param_->no_text_)
    vis_->addText("filtered hypotheses", 10, 10, vis_param_->fontsize_, vis_param_->text_color_[0],
                  vis_param_->text_color_[1], vis_param_->text_color_[2], "filtered_hypotheses", vp4_);
