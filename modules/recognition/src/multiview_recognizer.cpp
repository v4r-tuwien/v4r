    ...
    for (const ObjectHypothesesGroup &ohg_tmp : v_old.obj_hypotheses_) {
      ...
      if (do_hyp_to_transfer || !param_.transfer_only_verified_hypotheses_) {
        ...
        for (const ObjectHypothesis::Ptr &oh_tmp : ohg_tmp.ohs_) {
          ...
          //                    oh_copy->transform_ = v_old.camera_pose_ * oh_copy->transform_; ///< ATTENTION: This
          //                    depends on the input cloud (i.e. in this case the input cloud is in the global reference
          //                    frame)
      ...
      const View &v_old = views_[v_id];
      {
        ...
        *scene_cloud_normals_merged_ += scene_normals_aligned;
        ...
        for (const auto &oh : v_old.local_obj_hypotheses_)  // iterate through all models
        {
          ...
          for (pcl::Correspondence &c :
               new_corrs)  // add appropriate offset to correspondence index of the model keypoints
          {
            //                        CHECK( c.index_match < (int) scene_cloud_xyz_aligned.points.size() &&
            //                        c.index_match >= 0 ) << "c.index_match: " << c.index_match << ",
            //                        scene_cloud_xyz_merged_->points.size(): " <<
            //                        scene_cloud_xyz_merged_->points.size();
            //                        CHECK( c.index_match < (int) scene_normals_aligned.points.size() && c.index_match
            //                        >= 0 );
          ...
          //                    for (const pcl::Correspondence &c : new_corrs) // add appropriate offset to
          //                    correspondence index of the model keypoints
          //                    {
          //                        CHECK( c.index_match < (int) scene_cloud_xyz_merged_->points.size() && c.index_match
          //                        >= 0 ) << "c.index_match: " << c.index_match << ",
          //                        scene_cloud_xyz_merged_->points.size(): " << scene_cloud_xyz_merged_->points.size();
          //                        CHECK( c.index_match < (int) scene_cloud_normals_merged_->points.size() &&
          //                        c.index_match >= 0 );
          //                        CHECK( c.index_query < (int) model_keypoints->points.size() && c.index_query >= 0 );
          //                        CHECK( c.index_query < (int) model_kp_normals->points.size() && c.index_query >= 0
          //                        );
          ...
          if (it_mp_oh == local_obj_hypotheses_.end())  // no feature correspondences exist yet
            ...
            for (size_t new_corr_id = 0; new_corr_id < new_corrs.size(); new_corr_id++) {
              ...
              const Eigen::Vector3f &new_scene_xyz =
                  scene_cloud_xyz_merged_->points[new_c.index_match].getVector3fMap();
              const Eigen::Vector3f &new_scene_normal =
                  scene_cloud_normals_merged_->points[new_c.index_match].getNormalVector3fMap();
              const Eigen::Vector3f &new_model_xyz = model_keypoints->points[new_c.index_query].getVector3fMap();
              const Eigen::Vector3f &new_model_normal =
              ...
              for (size_t old_corr_id = 0; old_corr_id < old_corrs.size(); old_corr_id++) {
                ...
                const Eigen::Vector3f &old_scene_xyz =
                    scene_cloud_xyz_merged_->points[old_c.index_match].getVector3fMap();
                const Eigen::Vector3f &old_scene_normal =
                    scene_cloud_normals_merged_->points[old_c.index_match].getNormalVector3fMap();
                const Eigen::Vector3f &old_model_xyz = model_keypoints->points[old_c.index_query].getVector3fMap();
                const Eigen::Vector3f &old_model_normal =
                    model_kp_normals->points[old_c.index_query].getNormalVector3fMap();
...
template <typename PointT>
void MultiviewRecognizer<PointT>::visualize() {
  size_t counter = 0;
  ...
  for (it = local_obj_hypotheses_.begin(); it != local_obj_hypotheses_.end(); ++it) {
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr scene_cloud_xyz_merged_vis(
        new pcl::PointCloud<pcl::PointXYZ>(*scene_cloud_xyz_merged_));
    ...
    for (size_t c_id = 0; c_id < loh.model_scene_corresp_->size(); c_id++) {
      ...
      mn.getNormalVector3fMap() =
          model_keypoints_[model_id]->kp_normals_->points.at(c.index_query).getNormalVector3fMap();
    ...
    {
      std::sort(loh.model_scene_corresp_->begin(), loh.model_scene_corresp_->end(),
                LocalObjectHypothesis<PointT>::gcGraphCorrespSorter);
    ...
    LOG(INFO) << "visualizing " << model_kps_colored->points.size() << " and " << scene_kps_colored->points.size()
              << " for " << model_id;
    vis->addPointCloud(model_kps_colored, "model_kps", vp1);
    vis->addPointCloud(scene_kps_colored, "scene_kps", vp2);
    vis->addPointCloudNormals<PointT, pcl::Normal>(model_kps_colored, model_kps_normals, 1, 0.02f, "model_kp_normals",
                                                   vp1);
    vis->addPointCloudNormals<PointT, pcl::Normal>(scene_kps_colored, scene_kps_normals, 1, 0.02f, "scene_kp_normals",
                                                   vp2);
...
void MultiviewRecognizer<PointT>::correspondenceGrouping(const std::vector<std::string> &model_ids_to_search) {
  ...
  for (it = local_obj_hypotheses_.begin(); it != local_obj_hypotheses_.end(); ++it) {
    ...
    for (size_t cluster_id = 0; cluster_id < corresp_clusters.size(); cluster_id++)
      t_est.estimateRigidTransformation(*model_keypoints, *scene_cloud_xyz_merged_, corresp_clusters[cluster_id],
    ...
    if (param_.merge_close_hypotheses_) {
      ...
      for (size_t tf_id = 0; tf_id < new_transforms.size(); tf_id++) {
        ...
        for (size_t j = tf_id + 1; j < new_transforms.size(); j++) {
          ...
          double roty =
              std::abs(atan2(-rot_diff(2, 0), sqrt(rot_diff(2, 1) * rot_diff(2, 1) + rot_diff(2, 2) * rot_diff(2, 2))));
          double rotz = std::abs(atan2(rot_diff(1, 0), rot_diff(0, 0)));
          double dist = (centroid1 - centroid2).norm();

          if ((dist < param_.merge_close_hypotheses_dist_) && (rotx < angle_thresh_rad) && (roty < angle_thresh_rad) &&
              (rotz < angle_thresh_rad)) {
            merged_corrs.insert(merged_corrs.end(), corresp_clusters[j].begin(), corresp_clusters[j].end());
            cluster_has_been_taken[j] = true;
          }
        }

        t_est.estimateRigidTransformation(*model_keypoints, *scene_cloud_xyz_merged_, merged_corrs,
                                          merged_transforms[kept]);
        kept++;
      }
