 ...
 */
IMKOptimizeModel::IMKOptimizeModel(const Parameter &p, const v4r::FeatureDetector::Ptr &_detector,
 ...
 * @param pose
 */
void IMKOptimizeModel::addPoints3d(const std::vector<cv::KeyPoint> &_keys,
 ...
 * @return
 */
bool IMKOptimizeModel::loadObjectIndices(const std::string &_filename, cv::Mat_<unsigned char> &_mask,
                                         const cv::Size &_size) {
  int idx;
 ...
 */
bool IMKOptimizeModel::estimatePose(const View &view0, const View &view1,
                                    const std::vector<std::vector<cv::DMatch>> &matches, Eigen::Matrix4f &pose,
                                    std::vector<int> &_inliers) {
  cv_points_3d.clear();
  im_points.clear();
  depth_values.clear();
 ...
 */
bool IMKOptimizeModel::validatePoseAndAddLinks(View &view0, View &view1,
                                               const std::vector<std::vector<cv::DMatch>> &_matches01,
                                               const std::vector<std::vector<cv::DMatch>> &_matches10,
                                               const Eigen::Matrix4f &pose01, const Eigen::Matrix4f &pose10,
                                               const std::vector<int> &_inliers01, const std::vector<int> &_inliers10) {
  Eigen::Matrix4f inv_pose10;
  invPose(pose10, inv_pose10);

  if ((pose01.block<3, 1>(0, 3) - inv_pose10.block<3, 1>(0, 3)).norm() > param.eq_pose_dist || view0.keys.size() == 0 ||
      view1.keys.size() == 0)
    return false;

  view0.links.push_back(
      Link(view1.idx_part, view1.idx_view, ((double)_matches01.size()) / (double)view0.keys.size(), pose01));
  view1.links.push_back(
      Link(view0.idx_part, view0.idx_view, ((double)_matches10.size()) / (double)view1.keys.size(), pose10));
...
void IMKOptimizeModel::matchViews() {
  ...
  for (unsigned i = 0; i < object_views.size(); i++) {
    for (unsigned j = 0; j < object_views[i].size(); j++) {
      ...
      cv::Mat im0 =
          cv::imread(base_dir + std::string("/") + object_names[view0.idx_part] + std::string("/views/") + file,
                     CV_LOAD_IMAGE_COLOR);
      cv::imshow("im0", im0);
      // --
      for (unsigned k = 0; k < object_views.size(); k++) {
        for (unsigned l = 0; l < object_views[k].size(); l++) {
          ...
          if (i != k || j != l) {
            ...
            // cout<<" matches: "<<matches01.size()<<", "<<matches10.size()<<" -> "<<inliers01.size()<<",
            // "<<inliers10.size()<<endl;
          ...
          if (have) {
            ...
            boost::replace_last(file, "cloud_", "image_");
            cv::Mat im1 =
                cv::imread(base_dir + std::string("/") + object_names[view1.idx_part] + std::string("/views/") + file,
                           CV_LOAD_IMAGE_COLOR);
            cv::imshow("im1", im1);
            cv::waitKey(0);
          }
