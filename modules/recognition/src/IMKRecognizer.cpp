...
inline bool cmpObjectsDec(const v4r::triple<std::string, double, Eigen::Matrix4f> &a,
                          const v4r::triple<std::string, double, Eigen::Matrix4f> &b) {
 ...
 */
IMKRecognizer::IMKRecognizer(const Parameter &p, const v4r::FeatureDetector::Ptr &_detector,
 ...
 * @param pose
 */
void IMKRecognizer::addView(const unsigned &idx, const std::vector<cv::KeyPoint> &_keys, const cv::Mat &_descs,
                            const pcl::PointCloud<pcl::PointXYZRGB> &cloud, const cv::Mat_<unsigned char> &mask,
                            const Eigen::Matrix4f &pose, Eigen::Vector3d &centroid, unsigned &cnt) {
  ...
  for (unsigned i = 0; i < _keys.size(); i++) {
    ...
    if ((int)key.pt.x >= 0 && (int)key.pt.x < (int)cloud.width && (int)key.pt.y >= 0 &&
        (int)key.pt.y < (int)cloud.height) {
 ...
 * @param view
 */
void IMKRecognizer::setViewDescriptor(const std::vector<cv::Mat_<unsigned char>> &_im_channels,
                                      const pcl::PointCloud<pcl::PointXYZRGB> &cloud,
                                      const cv::Mat_<unsigned char> &mask, const Eigen::Matrix4f &pose, IMKView &view) {
  ...
  for (int v = 0; v < (int)mask.rows; v++) {
    for (int u = 0; u < (int)mask.cols; u++) {
      if (mask(v, u) > 128) {
        ...
        if (v > max_y)
          max_y = v;
        if (cloud(v, u).z > max_val)
          max_val = cloud(v, u).z;
        if (cloud(v, u).z < min_val)
          min_val = cloud(v, u).z;
      }
    }
  }
  ...
  if (min_y > 0)
    min_y--;
  if (max_x < mask.cols - 1)
    max_x++;
  if (max_x < mask.rows - 1)
    max_y++;
  ...
  for (unsigned i = 0; i < _im_channels.size(); i++) {
    cv::resize(_im_channels[i](cv::Rect(min_x, min_y, depth.cols, depth.rows)), im_roi_scaled,
               cv::Size(param.image_size_conf_desc, param.image_size_conf_desc));
 ...
 * @return
 */
bool IMKRecognizer::loadObjectIndices(const std::string &_filename, cv::Mat_<unsigned char> &_mask,
                                      const cv::Size &_size) {
  int idx;
...
void IMKRecognizer::createObjectModel(const unsigned &idx) {
  ...
  std::vector<std::string> cloud_files =
      v4r::io::getFilesInDirectory(base_dir + std::string("/") + name + std::string("/views/"), pattern, false);
  ...
  for (unsigned i = 0; i < cloud_files.size(); i++) {
    ...
    if (pcd.read(base_dir + std::string("/") + name + std::string("/views/") + cloud_files[i], *cloud2, origin,
                 orientation, version) < 0)
      continue;

    mask = cv::imread(base_dir + std::string("/") + name + std::string("/views/") + mask_file, CV_LOAD_IMAGE_GRAYSCALE);

    if (mask.empty()) {
      if (!loadObjectIndices(base_dir + std::string("/") + name + std::string("/views/") + object_indices_file, mask,
                             cv::Size(cloud2->width, cloud2->height)))
        continue;
    }
    ...
    cout << "Load " << (name + std::string("/") + cloud_files[i]) << ": detected " << key_cnt - tmp_nb << " keys"
         << endl;
  ...
  for (unsigned i = 0, z = start_frame; i < poses.size(); i++, z++) {
    ...
    if (dist_coeffs.empty())
      ...
      projectPointToImage(&pt[0], &intrinsic(0, 0), &dist_coeffs(0, 0), &center.x);
 ...
 * @param inliers
 */
int IMKRecognizer::getMaxViewIndex(const std::vector<IMKView> &views, const std::vector<cv::DMatch> &_matches,
 ...
 */
void IMKRecognizer::getNearestNeighbours(const Eigen::Vector2f &pt, const std::vector<cv::KeyPoint> &_keys,
                                         const float &sqr_inl_radius_conf, std::vector<int> &nn_indices) {
  nn_indices.clear();
  for (unsigned i = 0; i < _keys.size(); i++) {
 ...
 * @return
 */
double IMKRecognizer::computeGradientHistogramConf(const std::vector<cv::Mat_<unsigned char>> &_im_channels,
                                                   const IMKView &view, const Eigen::Matrix4f &pose) {
  // kp::ScopeTime tc("IMKRecognizer::computeGradientHistogramConf");
  if (view.conf_desc.size() == 0 || view.cloud.rows < 5 || view.cloud.cols < 5 ||
      view.weight_mask.rows != param.image_size_conf_desc || view.weight_mask.cols != param.image_size_conf_desc)
    throw std::runtime_error(
        "[IMKRecognizer::computeGradientHistogramConf] The object model does not fit to the current configuration! "
        "Please rebuild the model (delete the auto-generated model folder)!");
  if (_im_channels.size() == 0)
    return 0.;
...
#ifdef DEBUG_AR_GUI
    if (!view.im_gray.empty())
      cv::imshow("view.im_gray", view.im_gray);
 ...
 * @param clusters
 */
void IMKRecognizer::poseEstimation(
  ...
  for (int i = 0; i < (int)_clusters.size() && i < param.use_n_clusters; i++) {
    ...
    if (nb_ransac_trials < (int)param.pnp_param.max_rand_trials) {
      ...
      // );
      //      double conf = (view_idx>=0 && view_idx<(int)views.size()? (views[view_idx].keys.size()>0?
      //      ((double)inliers.size())/(double)views[view_idx].keys.size() : 0.) : 0.);
      double conf = computeGradientHistogramConf(_im_channels, views[view_idx], pose);
      objects.push_back(v4r::triple<std::string, double, Eigen::Matrix4f>(_object_names[ms.first],
                                                                          (conf > 1 ? 1. : conf < 0 ? 0 : conf), pose));
    }

#ifdef DEBUG_AR_GUI
    //    cout<<i<<": object_name="<<object_names[ms.first]<<",
    //    nb_ransac_trials="<<nb_ransac_trials<<"/"<<param.pnp_param.max_rand_trials<<(nb_ransac_trials==(int)param.pnp_param.max_rand_trials?"
    //    failed":" converged!!")<<endl;
    if (!dbg.empty()) {
      if (inliers.size() < param.min_cluster_size || nb_ransac_trials == (int)param.pnp_param.max_rand_trials)
        continue;
 ...
 * detect
 */
void IMKRecognizer::recognize(const cv::Mat &_image,
                              std::vector<v4r::triple<std::string, double, Eigen::Matrix4f>> &objects) {
  objects.clear();
 ...
 * detect
 */
void IMKRecognizer::recognize(const pcl::PointCloud<pcl::PointXYZRGB> &_cloud,
                              std::vector<v4r::triple<std::string, double, Eigen::Matrix4f>> &objects) {
  objects.clear();
