...
void GlobalRecognizer<PointT>::validate() const {
  CHECK(m_db_);
  CHECK(estimator_);
  CHECK(estimator_->getFeatureType() != FeatureType::OURCVFH || classifier_->getType() == ClassifierType::KNN)
      << "OurCVFH needs to obtain best training view - therefore only available in combination with nearest neighbor "
         "search";
  CHECK(classifier_->getType() == ClassifierType::KNN || param_.use_table_plane_for_alignment_ ||
        !param_.estimate_pose_);
}
  ...
  for (const typename Model<PointT>::ConstPtr m : models) {
    ...
    if (retrain || !io::existsFile(signatures_path)) {
      ...
      for (const auto &tv : training_views) {
        ...
        if (tv->cloud_)  // point cloud and all relevant information is already in memory (fast but needs a much memory
                         // when a lot of training views/objects)
        {
          scene_ = tv->cloud_;
        ...
        if (!similar_pose_exists) {
          ...
          std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>> model_poses_tmp(signature_tmp.rows(),
                                                                                                  pose);
          std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>> eigen_based_pose_tmp(
              signature_tmp.rows(), cluster_->eigen_pose_alignment_);

          gom->model_poses_.insert(gom->model_poses_.end(), model_poses_tmp.begin(), model_poses_tmp.end());
          gom->eigen_based_pose_.insert(gom->eigen_based_pose_.end(), eigen_based_pose_tmp.begin(),
                                        eigen_based_pose_tmp.end());

          gom->model_elongations_.conservativeResize(gom->model_elongations_.rows() + signature_tmp.rows(), 3);
          gom->model_elongations_.bottomRows(signature_tmp.rows()) =
              cluster_->elongation_.transpose().replicate(signature_tmp.rows(), 1);

          gom->model_centroids_.conservativeResize(gom->model_centroids_.rows() + signature_tmp.rows(), 4);
          gom->model_centroids_.bottomRows(signature_tmp.rows()) =
              cluster_->centroid_.transpose().replicate(signature_tmp.rows(), 1);

          gom->model_signatures_.conservativeResize(gom->model_signatures_.rows() + signature_tmp.rows(),
                                                    signature_tmp.cols());
          gom->model_signatures_.bottomRows(signature_tmp.rows()) = signature_tmp;

          // for OUR-CVFH
          std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>> descriptor_transforms =
              estimator_->getTransforms();
          gom->descriptor_transforms_.insert(gom->descriptor_transforms_.end(), descriptor_transforms.begin(),
                                             descriptor_transforms.end());
#ifdef _VISUALIZE_
      ...
      // compute the average discrepancy between the centroid computed on the whole 3D model to the centroid computed on
      // individual 2.5D training views.
    ...
    if (gom->model_signatures_.rows()) {
      all_model_signatures.conservativeResize(all_model_signatures.rows() + gom->model_signatures_.rows(),
                                              gom->model_signatures_.cols());
      ...
      all_trained_model_labels.tail(gom->model_signatures_.rows()) =
          target_id * Eigen::VectorXi::Ones(gom->model_signatures_.rows());
...
void GlobalRecognizer<PointT>::featureEncodingAndMatching(const std::vector<std::string> &model_ids_to_search) {
                                              ...
                                              // the computed SGURF (semi-global unique reference frame)
  {
    ...
    for (int query_id = 0; query_id < predicted_label.rows(); query_id++) {
      for (int k = 0; k < predicted_label.cols(); k++) {
        ...
        CHECK(it != gomdb_.global_models_.end())
  ...
  } else  // estimate pose using some prior assumptions
  {
    CHECK(!param_.use_table_plane_for_alignment_ ||
          (param_.use_table_plane_for_alignment_ && cluster_->isTablePlaneSet()))
        << "Selected to use table plane for pose alignment but table plane has not been set! " << std::endl;

    Eigen::Matrix4f tf_rot = Eigen::Matrix4f::Identity();

    if (param_.use_table_plane_for_alignment_)  // rotate cluster such that the surface normal of the planar support is
                                                // aligned with the z-axis. We do not need to know the closest training
                                                // view.
    {
      // create some arbitrary coordinate system on table plane (s.t. normal corresponds to z axis, and others are
      // orthonormal)
      Eigen::Vector3f vec_z = cluster_->table_plane_.topRows(3);
      vec_z.normalize();

      Eigen::Vector3f dummy;  /// NOTE we just need to find any other point on the plane except centroid to create a
                              /// coordinate system (hopefully this one is not close to zero)
      dummy(0) = 1;
      dummy(1) = 0;
      dummy(2) = 0;
    ...
    for (int query_id = 0; query_id < predicted_label.rows(); query_id++) {
      for (int k = 0; k < predicted_label.cols(); k++) {
        ...
        const Eigen::Vector3f &elongations_model =
            gom->model_elongations_.colwise().maxCoeff();  // as we don't know the view, we just take the maximum extent
                                                           // of each axis over all training views
        ...
        if (param_.use_table_plane_for_alignment_ ||
            classifier_->getType() != ClassifierType::KNN) {  // do brute force sampling along azimuth angle
          ...
          Eigen::Vector3f closest_pt_to_cluster_center =
              getClosestPointOnPlane(centroid_corrected, cluster_->table_plane_);
          ...
          tf_cluster_rot.block<3, 3>(0, 0) = computeRotationMatrixToAlignVectors(
              cluster_->table_plane_.head(3), Eigen::Vector3f::UnitZ());  // Finv * G * F;
          ...
          if (!vis_param_->no_text_) {
            vis.addText("object in model coordinate system", 10, 10, vis_param_->fontsize_, vis_param_->text_color_[0],
                        vis_param_->text_color_[1], vis_param_->text_color_[2], "object in model coordinate system",
                        vp1);
            vis.addText("model coordinate system aligned with centroid", 10, 10, vis_param_->fontsize_,
                        vis_param_->text_color_[0], vis_param_->text_color_[1], vis_param_->text_color_[2],
                        "model coordinate system aligned with centroid", vp2);
            vis.addText("model coordinate system aligned with downprojected centroid", 10, 10, vis_param_->fontsize_,
                        vis_param_->text_color_[0], vis_param_->text_color_[1], vis_param_->text_color_[2],
                        "model downprojected", vp3);
            vis.addText("sampled rotation angles", 10, 10, vis_param_->fontsize_, vis_param_->text_color_[0],
                        vis_param_->text_color_[1], vis_param_->text_color_[2], "sampled rotation angles", vp4);
            vis.addText("original cluster", 10, 10, vis_param_->fontsize_, vis_param_->text_color_[0],
                        vis_param_->text_color_[1], vis_param_->text_color_[2], "original cluster", vp5);
            vis.addText("origin aligned with cluster centroid", 10, 10, vis_param_->fontsize_,
                        vis_param_->text_color_[0], vis_param_->text_color_[1], vis_param_->text_color_[2],
                        "cluster centroid aligned with origin", vp6);
            vis.addText("origin aligned with downprojected cluster centroid", 10, 10, vis_param_->fontsize_,
                        vis_param_->text_color_[0], vis_param_->text_color_[1], vis_param_->text_color_[2],
                        "origin aligned with downprojected cluster centroid", vp7);
            std::stringstream correction_txt;
            correction_txt << "origin aligned with downprojected cluster centroid corrected by "
                           << (int)(1000.f * centroid_correction) << " mm.";
            vis.addText(correction_txt.str(), 10, 10, vis_param_->fontsize_, vis_param_->text_color_[0],
                        vis_param_->text_color_[1], vis_param_->text_color_[2],
                        "origin aligned with shifted downprojected cluster centroid", vp8);
          }
          ...
          typename pcl::PointCloud<PointT>::Ptr cluster_shifter_and_aligned_not_downprojected_not_corrected(
          ...
          pcl::transformPointCloud(*cluster, *cluster_shifter_and_aligned_not_downprojected_not_corrected,
                                   tf_cluster_rot * tf_cluster_wo_downprojection);
          Eigen::Vector4f min3d_tmp, max3d_tmp;
          pcl::getMinMax3D(*cluster_shifter_and_aligned_not_downprojected_not_corrected, min3d_tmp,
                           max3d_tmp);  /// TODO: Do this computation during initialization
          ...
          pcl::transformPointCloud(*cluster_shifter_and_aligned_not_downprojected_not_corrected,
                                   *cluster_shifter_and_aligned_not_downprojected_not_corrected,
                                   tf_cluster_shift_wo_down_wo_correction);
          vis.addPointCloud(cluster_shifter_and_aligned_not_downprojected_not_corrected,
                            "cluster_shifter_and_aligned_not_downprojected_not_corrected", vp6);
          vis.addCoordinateSystem(vis_param_->coordinate_axis_scale_,
                                  "cluster_shifter_and_aligned_not_downprojected_not_corrected_co", vp6);

          Eigen::Vector3f closest_pt_to_cluster_center_wo_correction =
              getClosestPointOnPlane(cluster_->centroid_.head(3), cluster_->table_plane_);
          ...
          pcl::transformPointCloud(*cluster, *cluster_shifted_and_aligned_wo_correction,
                                   tf_cluster_rot * tf_cluster_shift_wo_correction);
          vis.addPointCloud(cluster_shifted_and_aligned_wo_correction, "cluster_shifted_and_aligned_wo_correction",
                            vp7);
          vis.addCoordinateSystem(vis_param_->coordinate_axis_scale_, "cluster_shifted_and_aligned_wo_correction_co",
                                  vp7);
          ...
          if (keep_all_hypotheses_) {
            for (float rot_i = 0.f; rot_i < 360.f; rot_i += param_.z_angle_sampling_density_degree_) {
              ...
              tmp << "cluster_shifted_and_rotated_" << rot_i;
