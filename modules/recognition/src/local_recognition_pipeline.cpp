    ...
    for (auto &oh : local_hypotheses) {
      ...
      if (rec->getNumEstimators() > 1)  // check for redundancy (e.g. multi-scale feature matching)
      {
             ...
             new_corr_id++)  // add appropriate offset to correspondence index of the model keypoints
        {
          ...
          //                    );
          //                    CHECK( new_c.index_query < (int) model_keypoints->points.size() && new_c.index_query >=
          //                    0 );
          //                    CHECK( new_c.index_query < (int) model_kp_normals->points.size() && new_c.index_query >=
          //                    0 );
          ...
          for (size_t old_corr_id = 0; old_corr_id < kept; old_corr_id++) {
            ...
            //                        0 );
            //                        CHECK( exist_c.index_match < (int) scene_normals_->points.size() &&
            //                        exist_c.index_match >= 0 );
            //                        CHECK( exist_c.index_query < (int) model_keypoints->points.size() &&
            //                        exist_c.index_query >= 0 );
            //                        CHECK( exist_c.index_query < (int) model_kp_normals->points.size() &&
            //                        exist_c.index_query >= 0 );

            const Eigen::Vector3f &exist_scene_xyz = scene_->points[exist_c.index_match].getVector3fMap();
            const Eigen::Vector3f &exist_scene_normal =
                scene_normals_->points[exist_c.index_match].getNormalVector3fMap();
            const Eigen::Vector3f &exist_model_xyz = model_keypoints->points[exist_c.index_query].getVector3fMap();
            const Eigen::Vector3f &exist_model_normal =
                model_kp_normals->points[exist_c.index_query].getNormalVector3fMap();
