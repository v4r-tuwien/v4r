...
class V4R_EXPORTS MultiSessionModelling {
 private:
  ...
  float computeFSV(const typename pcl::PointCloud<PointT>::ConstPtr& cloud,
                   pcl::PointCloud<pcl::Normal>::ConstPtr& normals, const std::vector<int>& indices,
                   const Eigen::Matrix4f& pose, const typename pcl::PointCloud<PointT>::ConstPtr& range_image);
  ...
  inline void computeAbsolutePosesRecursive(
      Graph& grph_final, Vertex start, Vertex coming_from, Eigen::Matrix4f accum,
  ...
  void computeAbsolutePoses(Graph& grph_final,
                            std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>>& absolute_poses) {
 ...
 public:
  ...
  void setInputData(std::vector<typename pcl::PointCloud<PointT>::ConstPtr>& clouds,
                    ...
                    std::vector<std::vector<int>>& object_indices, std::vector<std::pair<int, int>>& session_ranges) {
    clouds_ = clouds;
    poses_ = poses;
