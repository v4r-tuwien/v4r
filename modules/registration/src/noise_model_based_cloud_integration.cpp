...
/**
 * @file noise_model_based_cloud_integration.cpp
...
void NMBasedCloudIntegration<PointT>::collectInfo() {
...
template <typename PointT>
void NMBasedCloudIntegration<PointT>::reasonAboutPts() {
  ...
  for (size_t i = 0; i < big_cloud_info_.size(); i++) {
    ...
    for (size_t cloud = 0; cloud < input_clouds_.size(); cloud++) {
      if (pt.origin == (int)cloud)  // we don't have to reason about the point with respect to the original cloud
        continue;

      // reproject point onto the cloud's image plane and check if its within FOV and if so, if it can be seen or is
      // occluded
