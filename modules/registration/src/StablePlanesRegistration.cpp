...
void v4r::Registration::StablePlanesRegistration<PointT>::mergeTriangles(
  ...
  for (size_t i = 0; i < mesh_out->polygons.size(); i++) {
    ...
    v1 = hull_cloud.points[mesh_out->polygons[i].vertices[1]].getVector3fMap() -
         hull_cloud.points[mesh_out->polygons[i].vertices[0]].getVector3fMap();
    v2 = hull_cloud.points[mesh_out->polygons[i].vertices[2]].getVector3fMap() -
         hull_cloud.points[mesh_out->polygons[i].vertices[0]].getVector3fMap();
  ...
  vis.addPointCloudNormals<pcl::PointNormal,pcl::PointNormal> (normal_center_cloud, normal_center_cloud, 1, 0.01,
  "normal_center_cloud");
  vis.spin();*/
}

template <class PointT>
void v4r::Registration::StablePlanesRegistration<PointT>::initialize(std::vector<std::pair<int, int>>& session_ranges) {
...
template <class PointT>
void v4r::Registration::StablePlanesRegistration<PointT>::compute(int s1, int s2) {
  ...
  typename pcl::PointCloud<ModelPointT>::Ptr target_model_on_plane(
      new pcl::PointCloud<ModelPointT>(*partial_models_with_normals_[s1]));
  ...
  for (size_t i = 0; i < std::min(MAX_PLANES_, stable_planes_[s2].size()); i++) {
    ...
    // ATTENTION: Make sure that the determinant is non-negative (meaning that we have an invertible rotation matrix,
    // otherwise weird flips...)
    ...
    while (rotation < 360.f) {
      Eigen::AngleAxisf rotation_z =
          Eigen::AngleAxisf(static_cast<float>(pcl::deg2rad(rotation)), Eigen::Vector3f::UnitZ());
      Eigen::Matrix4f pose;
      pose.setIdentity();
