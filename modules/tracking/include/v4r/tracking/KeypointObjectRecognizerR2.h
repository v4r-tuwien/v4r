...
class V4R_EXPORTS KeypointObjectRecognizerR2 {
  ...
  class V4R_EXPORTS Parameter {
   public:
    ...
    : inl_dist(_inl_dist), eta_ransac(_eta_ransac), max_rand_trials(_max_rand_trials), pnp_method(_pnp_method),
      nb_ransac_points(_nb_ransac_points), use_n_views(_use_n_views), cb_param(_cb_param) {}
  };
 ...
 public:
  ...
  KeypointObjectRecognizerR2(const Parameter &p, const v4r::FeatureDetector::Ptr &_detector,
 ...
 * cvToEigen
 */
inline void KeypointObjectRecognizerR2::cvToEigen(const cv::Mat_<double> &R, const cv::Mat_<double> &t,
                                                  Eigen::Matrix4f &pose) {
  pose.setIdentity();
